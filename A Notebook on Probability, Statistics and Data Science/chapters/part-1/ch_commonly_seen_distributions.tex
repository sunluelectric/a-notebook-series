...
\begin{eqnarray}
	...
	\end{array}\right. \nonumber \\
	&& \begin{array}{ll}
		\textup{s.t.} & 0\leq p \leq 1 \\
		& 0\leq q \leq 1 \\
		& p+q=1 \nonumber
	\end{array}
\end{eqnarray}
Each experiment is also called a \mync{Bernoulli trail}.
...
Bernoulli distribution can be taken as a special case of binomial distribution with $n=1$, $x=1$. The expectation, variance, skewness and excess kurtosis of the binomial distribution are $np$, $npq$, $\frac{q-p}{\sqrt{npq}}$ and $\frac{1-6pq}{npq}$, respectively.

If $n$ is large, Binomial distribution becomes a verification of CLT on Bernoulli trail. Let $n$ be a fixed large number, and let the Bernoulli trail describe the output to a stochastic system. Among the $n$ trials, $X$ of them outputs $1$ while $n-X$ of them, $0$. The mean of the output, therefore, is $X/n$. According to CLT, $X/n$ should follow normal distribution when $n$ is large, and in that case $X$ should also follow normal distribution. 

Indeed, when $n$ is large, binomial distribution approaches normal distribution.

Binomial distribution can be extended to \mync{multinomial distribution}, where instead of a single event $A$ happening with probability $p$ or not happening with probability $q$, s.t. $p+q=1$, consider multiple events $A_1$, $A_2$, ..., $A_m$, each with probability $p_1$, $p_2$, ..., $p_m$, respectively, subjected to $p_1+p_2+\ldots+p_m=1$. Consider a total of $n$ tests. The number of $A_1$ occurring is a random variable $X_1$, event $A_2$, $X_2$, and so on. Multinomial distribution studies the probability of
\begin{eqnarray}
...
On the other hand, if after each test the marble is not returned to the bag (this would suggest that $n\leq r+b$), it becomes a hypergeometric distribution and the probability follows
\begin{eqnarray}
...
For a random variable $X$ following normal distribution, the probabilities of its value falling between the mean $\pm \sigma$, $\pm 2\sigma$ and $\pm 3\sigma$ are $0.6827$, $0.9545$ and $0.9973$ respectively. In statistics, sometimes we will take samples with residuals larger than $|3\sigma|$ as outliers.
...
\end{figure}
The expectation, variance, skewness and excess kurtosis of the distribution are $\lambda$, $\lambda$, $\frac{1}{\sqrt{\lambda}}$ and $\frac{1}{\lambda}$, respectively. Notice that the sum of two Poisson distribution with shape parameters $\lambda_1$ and $\lambda_2$ is also Poisson distribution with $\lambda = \lambda_1 + \lambda_2$.

Poisson distribution is often used to describe the probability of an event happening a particular number of times, assuming that the occurrence of the event is independent, the chance of the event happening a single time is low, but there are many trials where the event can happen. The parameter $\lambda$ reflects the average number of occurrence. 

Poisson distribution can be taken as a limiting case of binomial distribution where $n\rightarrow\infty$, $p\rightarrow 0$, and $\lambda = np$ a decent value. A demonstration is given in Fig. \ref{fig:poisson_vs_b}.
\begin{figure}[!htb]
	\centering
...
\end{figure}
An intuitive explanation is given using the following example. Consider studying the number of failures of a machine in one year. In each second, the machine has a failure rate of $p=2\times 10^{-7}$ which is very small. The total number of seconds in a year is $31536000$, which is a large number and it is associated with the number of trails. When the machine fails, it automatically restarts instantly so that it would not affect its behavior in the consecutive second.

The number of failure of the machine in a year can be described by binomial distribution with $n=31536000$ a large number and $p=2\times 10^{-7}$ a small number. The expected number of failures is $np\approx 6.3$, and can be approximately described by a Poisson distribution with $\lambda=np\approx 6.3$.

Another example of Poisson distribution is the number of visitors of a website. Each internet user has a small probability $p$ to visit the website. The total number of internet users is $n$ which is a large number. The number of visitors to the website follows Poisson distribution with $\lambda=np$ being the expected visitor number.

\section{Exponential Distribution} \label{sec:exponential_distribution}

\mync{Exponential distribution} is a continuous distribution that can be used to model the duration of time between two events in a Poisson distribution. For example, it can model the operating time of a machine between two failures, assuming that the failures are independent and random.
...
Exponential distribution is an important special case of the $\Gamma$ distribution. More about $\Gamma$ distribution will be introduced in more detail in Section \ref{sec:gamma-distribution}
...
The expectation, variance, skewness and excess kurtosis of the distribution are $\frac{a+b}{2}$, $\frac{(b-a)^2}{12}$, $0$ and $-\dfrac{6}{5}$, respectively.
...
\section{$\Gamma$, $\chi^2$, $\beta$ and $F$ Distributions} \label{sec:gamma-distribution}

The \mync{$\Gamma$}[gamma], \mync{$\chi^2$}[chi-square], \mync{$\beta$}[beta] and \mync{$F$} distributions are introduced as follows.
...
\end{eqnarray}
for $\alpha > 0$. $\Gamma$ distribution is the superset of many distributions. For example, exponential distribution introduced earlier is a special case of $\Gamma$ distribution with $\alpha=1$, and it is used to model the time interval between two events in the Poisson distribution.
...
\end{eqnarray}
Equation \eqref{eq:chi2pdf} is also denoted by $\chi^2(r)$, where $r$ is known as the degrees of freedom. The plots of PDFs with different $r$ are shown in Fig. \ref{fig:chi2_pdf}.
\begin{figure}[!htb]
	\centering
...
The expectation, variance, skewness and excess kurtosis of the distribution are $r$, $2r$, $\frac{2\sqrt{2r}}{r}$ and $\frac{12}{r}$, respectively.
...
\end{eqnarray}
can be used and \eqref{eq:betapdf} can be re-written as
\begin{eqnarray}
	f(x) &=& \left\{\begin{array}{cc}
		\dfrac{1}{\beta(\alpha_1, \alpha_2)}x^{\alpha_1-1}(1-x)^{\alpha_2-1} & 0 < x < 1 \\
		0 & \textup{otherwise}
	\end{array}\right. \nonumber
\end{eqnarray}
...
\begin{eqnarray}
	f(x) &=& \left\{\begin{array}{cc}
		\dfrac{1}{\beta\left(\dfrac{\nu_1}{2},\dfrac{\nu_2}{2}\right)}\left(\dfrac{\nu_1}{\nu_2}\right)^{\dfrac{\nu_1}{2}}x^{\left(\dfrac{\nu_1}{2}+1\right)}\left(1+\dfrac{\nu_1}{\nu_2}x\right)^{\left(-\dfrac{\nu_1 + \nu_2}{2}\right)} & x > 0 \\
		0 & \textup{otherwise}
	\end{array}\right. \nonumber
\end{eqnarray}
where $\nu_1$, $\nu_2$ are the shape parameters and $\beta(\cdot)$ the beta function. 

\section{Student's $t$-Distribution}

The zero-mean \mync{Student's $t$-distribution} (also known as $t$-distribution for simplicity) PDF is given by
\begin{eqnarray}
...
The expectation, variance, skewness and excess kurtosis of the $t$-distribution given by \eqref{eq:tpdf} are $0$, $\frac{\nu}{\nu-2}$ $(\nu>2)$, $0$ $(\nu>3)$ and $\frac{6}{\nu-4}$ $(\nu>4)$ respectively.
