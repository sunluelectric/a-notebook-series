...
\mync{Randomness} typically refers to the case where a variable (as in ``random variable'') is not precisely measurable or predictable. However, the chance of it taking particular values may follow some patterns known as the statistical properties of the variable. For example, the reading of a sensor may contain random noise. The value of the noise corresponding with a particular reading is unknown, but the range of it is often known and can be obtained from the sensor manufacture. For another example, let variable $X$ denote the result of tossing a coin, and $X$ is a random variable with the following pattern. It is either head ($X=1$) or tail ($X=0$), each with a $50\%$ chance. The precise value remains unknown until the coin is tossed.
