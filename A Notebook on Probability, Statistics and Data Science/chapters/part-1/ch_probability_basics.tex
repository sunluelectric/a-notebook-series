...
This notebook mainly studies random variables. Stochastic process and control of stochastic systems are introduced elsewhere in control system related notebooks, such as in robust control related chapters.

\subsection{Random Experiment}

Experiments are very important science and engineering. Very often, experiments are used to confirm theories. In these cases, the experiments are carefully designed so that their outcomes are deterministic and predictable as long as a theory is correct. By observing the results of the experiments matching the prediction of the theory, we build confidence in the theory.

However, this notebook is concerned with another type of experiment where the result contains randomness due to the lack of information or incomplete modeling. Such experiments include tossing a coin, measuring the lifespan of a randomly picked item from a production line, etc. These experiments are known as \mync{random experiments}. The result of a random experiment is often unpredictable and sometimes meaningless. However, the collective results of a set of random experiments repeatedly carried out can be meaningful because they reflect some insights of the system. The challenge is to design the experiments so that useful information can be revealed from the experiment results efficiently.

\subsection{Sample Space}

A set $S$ that consists of all possible outcomes of a random experiment is called a \mync{sample space}.

Depending on the cardinality (number of elements in a set) of a sample space, it can be categorized as follows. If a sample space has a finite number of elements, it is called a \mync{finite sample space}. Otherwise, it is called an \mync{infinite sample space}. If the elements in an infinite sample space can be mapped to natural numbers, the sample space is also called a \mync{countably infinite sample space}. Otherwise, the sample space is called a \mync{uncountably infinite sample space}. Examples are given below.
\begin{itemize}
...
Parameters $h$ and $n$ are the cardinality of event $A$ and sample space $S$ respectively. Calculating $h$ and $n$ is the key to solving the classical probability. Permutation and combination are widely used for such calculations. 
...
This bridges classical probability and the axiomatic approach. Notice that geometric probability can be taken as an expansion of the classical probability and hence is also linked to the axiomatic approach. Classical probability and geometric probability are essentially probability function of axiomatic approach in certain applications.

\begin{mdframed}
\noindent \textbf{Axiomatic approach and Frequency Approach}
...
The chances of a child being a boy or a girl are the same half-half. Gather a group of couples with 2 children. Randomly pick one couple.
...
\begin{enumerate}
	...
	\item From an earlier interview, we know that their first child is a girl. What is the chance of them having two girls?
\end{enumerate}
...
\begin{eqnarray}
	P(A|C) &=& \dfrac{P\left(A \cap C\right)}{P(C)} \nonumber \\
	&=& \dfrac{P(A)}{P(C)} \nonumber \\
	&=& \dfrac{1}{2} \nonumber
\end{eqnarray}
Notice that the third question is essentially asking the probability of the second child being a girl, as we already know that the first child is a girl. Therefore, the probability can also be calculated using
\begin{eqnarray}
	P(D|C) &=& \dfrac{P\left(D \cap C\right)}{P(C)} \nonumber \\
	&=& \dfrac{P(A)}{P(C)} \nonumber \\
	&=& \dfrac{1}{2} \label{eq:probsetexpa}
\end{eqnarray}

From \eqref{eq:probsetexp} and \eqref{eq:probsetexpa}, we can see that $P(D|C) = P(D) = \frac{1}{2}$, which indicates that the prior information about whether $C$ occurred does not provide additional insights on whether $D$ occurred, as $D$ always has a chance of $50\%$ to occur regardless of the result of $C$. This makes sense. The genders of both children are independent. The first child, whether a boy or a girl, does not affect the likelihood of the gender of the second child.
...
\begin{eqnarray}
	f(x) &=& \dfrac{d}{dx}F(x) \nonumber
\end{eqnarray}
With this definition, $f(x)$ is such function that
\begin{eqnarray}
...
Apparently from the axiom,
\begin{eqnarray}
	\int_{-\infty}^{\infty}f(x)dx &=& 1 \label{eq:pdf-axiom}
\end{eqnarray}
for any continuous random variable.
...
Let $X$ be a random variable with PDF $f_X(x)$. Let $U$ be another random variable which is a function of $X$, $U=\phi(X)$. The PDF of $U$ can be calculated from $f_X$ and $\phi$ as follows.
...
Given the probability function of a discrete random variable or the PDF of a continuous random variable, a lot of insights can be extracted. Commonly seen measures of a random variable are introduced in this and the next sections.

In probability and statistics, \mync{expectation} (also known as \mync{mean}) describes the average value of a random variable, if the experiment is carried out many times. For discrete random variable $X$, the expectation is given by
\begin{eqnarray}
	\textup{E}(X) &=& \sum_{i=1}^{n}x_iP(x_i) \label{eq:expectationdiscrete}
\end{eqnarray}
where $\textup{E}(\cdot)$ denotes the expectation, and $n$ the cardinality of the sample space. In the case of countably infinite sample space, replace $n$ with $\infty$. For continuous random variable $X$, the expectation is given by
\begin{eqnarray}
	\textup{E}(X) &=& \int_{-\infty}^{\infty}xf(x)dx \label{eq:expectationcontinuous}
\end{eqnarray}
Expectation is sometimes denoted by $\mu$ in literatures.

Expectation has the following commonly used features.
\begin{eqnarray}
	\textup{E}(cX) &=& c\textup{E}(X) \nonumber \\
	\textup{E}(X+Y) &=& \textup{E}(X) + \textup{E}(Y) \nonumber
\end{eqnarray}
where $c$ is a constant and $X$, $Y$ two random variables. These features can be easily derived using \eqref{eq:expectationdiscrete} and \eqref{eq:expectationcontinuous}. 
...
\end{eqnarray}
Variance and standard deviation are non-negative and often denoted by $\sigma^2$ and $\sigma$ respectively. Notice that \eqref{eq:varderived} also implies that $\textup{E}\left(X^2\right) \geq \textup{E}(X)^2$, a conclusion used in many lemma derivations.

For continuous random variables, from \eqref{eq:vardef} the variance is
\begin{eqnarray}
	\textup{Var}(X) &=& \int_{-\infty}^{\infty} (x-\textup{E}(X))^2f(x)dx \label{eq:varcontinuousf}
\end{eqnarray}

Variance has the following commonly used features.
\begin{eqnarray}
	\textup{Var}(cX) &=& c^2\textup{Var}(X) \nonumber \\
	\textup{Var}(X\pm Y) &=& \textup{Var}(X) + \textup{Var}(Y) \label{eq:variance-sum-independent}
\end{eqnarray}
where in \eqref{eq:variance-sum-independent} $X$ and $Y$ are independent random variables.
...
In mathematics, the \mync{$n$-th order moment} of a continuous function $f(x)$ about $c$ is defined as follows.
\begin{eqnarray}
	\mu_n &=& \int_{-\infty}^{\infty}(x-c)^nf(x)dx \nonumber
\end{eqnarray}

By simply saying ``moment'' without specifying $c$, $c=0$ by default. Let $f(x)$ be a PDF. With \eqref{eq:pdf-axiom} and \eqref{eq:expectationcontinuous}, the $0$-th and the $1$st order moments of a probability density function are
\begin{eqnarray}
...
Further more, let $c=\mu_1$ be the mean of the random variable. With \eqref{eq:varcontinuousf}, the $2$nd order central moment $\mu_2$ about $c$ is
\begin{eqnarray}
	\mu_2 = \int_{-\infty}^{\infty}(x-\mu_1)^2f(x)dx = \textup{Var}(X) \nonumber
\end{eqnarray}
which is the variance of the random variable.

Define the $3$-rd and $4$-th order standardized moments as follows. Let $\bar{\mu}_k$, $k = 3, 4$ be given by
\begin{eqnarray}
...
Equation \eqref{eq:jointpdfdowngrade} can be interpreted as follows. If $(X_i, Y_i)$ samples are generated from $f(x, y)$ in \eqref{eq:jointpdf}, and we only look at first element $X_i$ of these samples, they should follow \eqref{eq:jointpdfdowngrade}.

Using \eqref{eq:jointpdfdowngrade}, the expectation and variance for a variable can be calculated from the joint PDF as follows.
\begin{eqnarray}
...
\subsection{Distribution of Summation of Random Variables}
...
\subsection{Expectation of Multiplication of Random Variables}
...
\end{eqnarray}
where $f(x, y)$ is the joint distribution of $X$ and $Y$. Furthermore, if $X$, $Y$ are independent,
\begin{eqnarray}
	\textup{E}(XY) &=& \int_{-\infty}^{\infty}\int_{-\infty}^{\infty} xyf(x, y)dxdy \nonumber \\
	&=& \int_{-\infty}^{\infty}xf_X(x)dx \times \int_{-\infty}^{\infty}yf_Y(y)dy \nonumber \\
	&=& \textup{E}(X)\textup{E}(Y) \nonumber
\end{eqnarray}
where $f_X(x)$, $f_Y(y)$ are the distributions of $X$ and $Y$ which can be derived from $f(x,y)$.
...
In \eqref{eq:variance-sum-independent}, the variance of the sum of random variables are given. In the case where the variables are dependent, \eqref{eq:variance-sum-independent} shall be generalized to
\begin{eqnarray}
	\textup{Var}(X\pm Y) &=& \iint_{-\infty}^{\infty} \left(X + Y - E\left[X + Y\right]\right)f(x,y)dxdy \nonumber \\
	&=& \iint_{-\infty}^{\infty} \left(X - E\left[X\right]\right)f(x,y)dxdy + \iint_{-\infty}^{\infty} \left(Y - E\left[Y\right]\right)f(x,y)dxdy \nonumber \\
	&& + 2\iint_{-\infty}^{\infty} \left(X - E\left[X\right]\right)\left(Y - E\left[Y\right]\right)f(x,y)dxdy \nonumber \\
	&=& \textup{Var}(X) + \textup{Var}(Y) + 2\textup{Cov}(X, Y) \nonumber
\end{eqnarray}
...
or equivalently
\begin{eqnarray}
	\sigma_{XY}^2 &\leq& \sigma_X^2\sigma_Y^2 \label{eq:covlemma-original}
\end{eqnarray}
Noticing that while $\sigma_X$, $\sigma_Y$ are always non-negative, $\sigma_{XY}$ is not, and \eqref{eq:covlemma-original} can be rewritten as
\begin{eqnarray}
...
The \mync{Law of Large Numbers}[LLN] is a theorem that says if performing the same experiment a large number of times, the average of the outcomes of the experiments should eventually converge to a certain value which is the expectation of the experiment.
...
LLN bridges analytical conclusion and empirical experiment results.

\subsection{Central Limit Theorem}

\mync{Central Limit Theorem}[CLT] states the following observation. For \myabb{independent and identically distributed}{i.i.d.} random variables not necessarily following normal distribution, the empirical mean of the samples taken from these distributions tends towards normal distribution when the number of samples is large. More about normal distribution will be introduced in Section \ref{sec:normal-distribution}; for now, take it as a specific distribution with predefined form of PDF.

Let $X$ be a random variable not necessarily following normal distribution, and it has mean and variance of $\mu$ and $\sigma^2<\infty$ respectively. Draw $n$ samples from the distribution. Let $X_i$ be the samples. The empirical mean of the samples is hence calculated by
\begin{eqnarray}
	\bar{X}_n &=& \dfrac{1}{n}\sum_{i=1}^{n}X_i \nonumber
\end{eqnarray}
Notice that according to LLN, if $n$ approaches infinity, $\bar{X}_n$ shall converge to the mean of the experiment. In practice, with limited though large $n$, $\bar{X}_n$ is a random variable. Imagine repeating the above several times and a set of $\bar{X}_n$ with different values can be obtained. 

The distribution of $\bar{X}_n$ depends on the original distribution as well as the choice of $n$. For example, if $n=1$, $\bar{X}_n = X_1$ and clearly it will follow the original distribution of $X$. 

CLT states that $\bar{X}_n$ tends towards normal distribution when $n$ is large, regardless of the original distribution of $X$.
