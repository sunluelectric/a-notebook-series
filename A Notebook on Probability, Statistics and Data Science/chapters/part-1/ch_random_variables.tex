...
The probability of $X$ taking a particular value $x$ is denoted by $P(X=x)$ or simply $f(x)=P(X)$ if no ambiguity. In this case, $P(X=x)$ and $f(x)$ are the \mync{probability function} (also known as \mync{probability mass function}) of $X$.
...
\mync{Joint probability} describes the distribution of two or more random variables. For simplicity, in the remaining of this section, two random variables $X$ and $Y$ are considered. The same idea can be expanded to more variables.
