...
This and the next chapter introduce R language, a widely used programming language for data mining and visualization. 

This chapter focuses on the fundamental setups, basic data types and basic visualization methods. The next Chapter \ref{ch:r2} introduces advanced data analysis tools and skills that R uses to solve practical problems.

\section{Brief Introduction} \label{ch:r1:sec:rinstallation}

\mync{R language} is a programming language for statistical computing and visualization. It is widely used among statisticians and data miners for developing statistical software and carrying out data analysis. When comes to statistics calculation and data visualization, R is often considered simpler and more efficient with large size datasets compared with other programming languages such as Python. R is free and can be downloaded from \cite{r} where more details about R can also be found.

\mync{RStudio}, also known as \mync{Posit}, is an \myabb{integrated development environment}{IDE} widely used for R programming and testing. RStudio IDE has an open-source and free-of-charge community version for personal use. It can be downloaded from \cite{rstudio}. Notice that RStudio is not compulsory and there are other IDEs that supports R script programming such as VSCode. That said, most of the R scripts in this notebook are programmed and tested with RStudio.

\subsection{Installation}

Download R and RStudio from \cite{r} and \cite{rstudio}, and install them following the corresponding instructions. It is required to install R before RStudio.

When working with R language, the user needs to maintain the version of R and its packages (more to be introduced in Section \ref{ch:r1:sec:rpackage}) and to make sure that they are compatible. Anaconda allows the user to create a virtual environment for a project and manage packages within that environment. For that reason, the user might prefer to use Anaconda to install and maintain R and its packages. The installation and maintenance of R via Anaconda can be found on \cite{anaconda2025r} and it is not covered in this notebook.

\subsection{R Packages Management} \label{ch:r1:sec:rpackage}

R uses packages, also known as libraries, to expand its capability. R packages, both built-in and third-party, provide powerful features for data analysis and visualization. Some packages come with demonstrative sample data frames. Packages can be published and shared online. \mync{CRAN} is by far the most popular platform to store and share R packages.

There are a number of built-in packages that come with R installation. Only third-party packages need separate installation, and it can be done using the R console. To install or remove a package, use
\begin{lstlisting}
...
It is a good practice to load a package before using its functions and datasets. To load a package, both built-in and third-party, use
\begin{lstlisting}
library(<package>)
\end{lstlisting}
For example
...
It is possible to use the functions and data frames in a package without loading it, in which case use prefix \verb|<package>::<function>|, \verb|<package>::<dataframe>|. This is inconvenient if the functions or data frames are to be used frequently.
...
\end{lstlisting}
The above command can be executed without loading \verb|pacman| in advance since \verb|pacman::p_load()| prefix is used. The listed packages are commonly used in R projects, and a brief explanation to each of them is given as comments in the above code. 

RStudio provides a graphical interface to manage packages as shown in Fig. \ref{ch:r1:fig:rstudio_package_management}. A package can be loaded or unloaded simply by checking or unchecking the corresponding checkbox.
\begin{figure}[!htb]
\centering
\includegraphics[width=350pt]{chapters/part-3/figures/rstudio_package_management.png}
\caption{RStudio's graphical interface for package management.} \label{ch:r1:fig:rstudio_package_management}
\end{figure}

\section{Basic Data Types and Syntax} \label{ch:r1:sec:rprogramming}

This section introduces the basic data types and their associated operations. Basic syntax such as conditionals, loops and user-defined functions are also illustrated.
...
Variable names are used as identifiers to variables. Legitimate variable names are a combination of letters, numbers, periods (\verb|.|) and underscores (\verb|_|). The name cannot start with a number or the underscore, but can start with the period. A name cannot contain dash (\verb|-|), space or special characters.

Note that it is different in R and in many other languages how period (\verb|.|) is interpreted when used in variable names. Many languages such as Python and MATLAB uses the period to indicate variables / methods of a class or fields of a structure. Take MATLAB as an example, where there could be a structure called ``\verb|student|'' with \verb|student.name|, \verb|student.age| the fields of \verb|student|. However, this is not the case with R. In R, \verb|student|, \verb|student.name| and \verb|student.age| are treated as different variables without logical connections. R uses \verb|<variable>$<field>| to indicate fields of a structure. More details will be given in later sections when list and data frame are introduced.
...
\begin{table}[!htb]
  ...
  \begin{tabularx}{\textwidth}{llX}
    ...
    double & \verb|x <- 2| & A double float value. \\
...
There is a rich set of supported functions for numerical calculations, string operations, probability calculations and statistics analysis. Some of them are summarized in Tables \ref{ch:r1:tab:numericalcalculation}, \ref{ch:r1:tab:logicalcomparison}, \ref{ch:r1:tab:stringoperation}, \ref{ch:r1:tab:probabilityfunction} and \ref{ch:r1:tab:statisticsfunction}. Notice that some of the functions may come from commonly used third-party packages.
...
\begin{table}[!htb]
  ...
  \begin{tabularx}{\textwidth}{lX}
    \hline
    Syntax & Description \\ \hline
    \verb|substr(s, n1, n2)| & Segment of a string, from the $n_1$-th character to $n_2$-th character, both characters included. Notice that the index starts from $1$ but not $0$. \\
...
\begin{table}[!htb]
	...
	\begin{tabularx}{\textwidth}{lX}
		...
		Syntax & Description \\ \hline
		\verb|mean()| & Mean. \\
		\verb|sd()| & Standard deviation. \\
		\verb|median()| & Median. \\
		\verb|range()| & Minimum and maximum. \\
		\verb|min()| & Minimum. \\
		\verb|max()| & Maximum. \\
		\verb|sum()| & Sum \\
		\hline
...
Vector and matrix are the fundamental forms using which R stores and processes data. List is similar with vector to some extend. Vector, matrix and list are introduced in this and following sections.
...
To append to a vector, or to add vectors to form a longer vector, or to insert a vector in the middle of a vector, use \verb|append()| as follows.
\begin{lstlisting}
> a <- c(1, 2, 3)
> a <- append(a, 4)
> print(a)
...
[1] "a"   "b"   "cde" "a"   "b"   "cde"
\end{lstlisting}
Replica can also be used to create empty vector with \verb|rep(NA, <repeat>)|.
...
To access the element in a vector, use \verb|<vector>[<index>]|, where \verb|<index>| can be an integer or a vector of integers. Examples are given below.
\begin{lstlisting}
> s <- c("a", "b", "c", "d", "e", "f", "g")
> s[1]
[1] "a"
...
Notice that \verb|NA| will be returned if the index used to access the element in a vector exceeds its length, or if the name used to access the element in a vector is not found. This is different from most programming languages which will usually raise an exception. To remove the name of a vector, simply assign \verb|NULL| to \verb|names(<vector>)|. 

And finally to return the length of a vector, use \verb|length(<vector>)|.

Vectorization operations significantly speed up the calculation comparing with looping over elements. Most numerical calculations, including \verb|+|, \verb|-|, \verb|*|, \verb|/|, \verb|%/%|, \verb|%%|, \verb|^|, support vectorization operations. Examples are given below.
\begin{lstlisting}
> a <- c(1,2,3,4,5)
> b <- c(5,4,3,2,1)
> a + b
...
\begin{mdframed}[frametitle={Two indexes methods: \texttt{[]} versus \texttt{[[]]}}]

So far we have been using \verb|<vector>[<index>]| to access elements in a vector, and it works alright. The \verb|<index>| can be a vector of indexes, in which case the return is also a vector. There is an alternative way of accessing elements, and that is \verb|<vector>[[<index>]]|. An example is given below.
...
From the above example, one may notice that for unnamed vectors \verb|[]| and \verb|[[]]| behave similarly when accessing a single element. For named vectors, \verb|[]| and \verb|[[]]| behave slightly differently. 

As introduced earlier, \verb|<index>| can be a vector in  \verb|<vector>[<index>]|. However this is not true for \verb|<vector>[[<index>]]|. An example is given below.
\begin{lstlisting}
> print(a[1:3])
first second  third 
1      2      3 
...
There are some semantic differences when using \verb|[]| and \verb|[[]]|. In short, \verb|[]| returns a sub-vector of the original vector (the length of the sub-vector can be 1), while \verb|[[]]| returns the element in the vector. This explains why it triggers an error when selecting multiple elements using \verb|[[]]|. 

Notice that numeric values are atomic in R, meaning that they behave the same whether they are extracted as a single-element vector or a number. That is why \verb|print(a[1])| shows the same result as \verb|print(a[[1]])| in the earlier example, except that \verb|[]| preserves the name of an element.

In the context of unnamed numeric vectors and matrices, \verb|[[]]| is rarely used, and \verb|[]| works just fine almost all the time. However, when comes to other data structures like list, etc., it is important to distinguish \verb|[]| and \verb|[[]]|, and choose the correct one depending on the purpose.

\end{mdframed}

\subsection{Matrix}

Similar with vector, all elements in a matrix must have a consistent datatype. A demonstration is shown in Fig. \ref{ch:r1:fig:r_table_demo} with elements indexes. Let the matrix be named \verb|A|. The elements in the matrix can be accessed by the name of the table followed by the index coordinates. For example, in the figure, \verb|A[1,1]| refers to the first element and \verb|A[4,6]| the last element. It is also possible to index an entire row or column. For example, use \verb|A[1,]| to represent the first row of the matrix.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=250pt]{chapters/part-3/figures/r_table_demo.png}
	\caption{A demonstration of a matrix in R.} \label{ch:r1:fig:r_table_demo}
\end{figure}

A matrix can be created from scratch by stacking row vectors using \verb|rbind()| as follows.
\begin{lstlisting}
# build rows
...
Similar with \verb|names(<vector>)| that can be used to view and assign names to elements in a vector, \verb|rownames(<matrix>)| and \verb|colnames(<matrix>)| can be used to view and assign names to the rows and columns of a matrix. Examples are given below.
\begin{lstlisting}
> A <- matrix(1:9, 3, 3)
...
By default, a single row or column of a matrix is automatically downgrade to a vector. An example is given below. When a matrix downgrades to a vector, the row name (if it has only one row), or the column name (if it has only one column) will be removed.
\begin{lstlisting}
> A <- matrix(1:9, 3, 3)
> is.matrix(A)
[1] TRUE
...
R provides flexible and powerful data visualization tools, many of which more advanced than the ones to be introduced in this section. This section introduces a simple matrix visualization function called \verb|matplot()|, which simply plots all the columns of a matrix. Notice that to plot the rows of a matrix, use $t(\cdot)$ to perform matrix transpose.
...
matplot(
	2018:2022, # x axis
	t(citation.ratio), # y axis, in this case citation.ratio rows
...
\end{lstlisting}
where \verb|t(\cdot)| used inside \verb|matplot()| calculates the transpose of a matrix. Execute the above to get Fig. \ref{ch:r1:fig:matplot_demo}.
\begin{figure}[!htb]
	\centering
	\includegraphics[width=250pt]{chapters/part-3/figures/matplot_demo.png}
	\caption{A demonstration of using \texttt{matplot} to plot trends.} \label{ch:r1:fig:matplot_demo}
\end{figure}

Another example of analyzing the performance of players through a series of basketball games are given below. Notice that in this example, user-defined function \verb|myplot()| is used. More about the syntax of defining a function will be introduced in Section \ref{sec:r-user-defined-function}.
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=250pt]{chapters/part-3/figures/basket_exp1.png}
	\caption{Plot of penalty success rate of the 3 players in 10 matches.} \label{ch:r1:fig:basket_exp1}
\end{figure}

\begin{figure}[!htb]
	\centering
	\includegraphics[width=250pt]{chapters/part-3/figures/basket_exp2.png}
...
As introduced earlier, there are two ways to index elements in a vector, matrix or list, namely \verb|[]| and \verb|[[]]|. While in the context of vectors and matrices \verb|[]| is almost always used, in the context of list \verb|[]| and \verb|[[]]| both have purposes and they can make a big difference. When retrieving a sub-list from the list, use \verb|[]|. When retrieving the content of a single element, use \verb|[[]]| or  \verb|<list>$<name>| which is simply a shorthand of \verb|<list>[[<name>]]|. Examples are given below.
\begin{lstlisting}
> print(list2$name)
[1] "lu"
...
\section{Data Frame} \label{ch:r1:sec:df}
...
Data frame can be created from scratch by concatenating columns just like matrix, and it is introduced in Section \ref{subsec:df-integration-of-data}. However, in practice it is more common that a data frame is obtained by importing data from other data sources. Hence, data importing is introduced first.
...
\subsection{Data Accessing}

There are a few ways to access an element in a data frame as shown below.
\begin{lstlisting}
<df>[<row-index>,<column-index>]
<df>[<row-index>,"<column-name>"]
<df>[,<column-name>][<row-index>]
<df>$<column-name>[<row-index>]
\end{lstlisting}
where the later two essentially adopt a 2-step procedure and
\begin{lstlisting}
<df>[, <column-name>]
<df>$<column-name>
\end{lstlisting}
are two equivalent ways of accessing the entire column in a data frame.
...
\begin{table}[!htb]
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    \verb|levels(df$<column>)| & The level of the column; only works for descrete-value columns. \\
    \hline
  \end{tabularx}
\end{table}
...
\begin{lstlisting}
<filter> <- <true-false-vector> # use true-false vector as filter
...
132          7.9         3.8          6.4         2.0 virginica
\end{lstlisting}
As shown above, it is possible to use \verb|&|, \verb$|$ to form a compound filter. The commands can be merged together as follows.
\begin{lstlisting}
...
\subsection{Data Frame from Integration of Data} \label{subsec:df-integration-of-data}
...
Conditional statement, loop statement and user-defined function are introduced.

\subsection{Conditional Statement}

The \verb|if| statement syntax is as follows.
\begin{lstlisting}
if(<condition>){
	<command>
} else if(<condition>){
...
\end{lstlisting}
An example of using \verb|if| statement is given below.
\begin{lstlisting}
> x <- rnorm(1)
> if(x > 0){
	+   y <- x
...
\subsection{User-Defined Function} \label{sec:r-user-defined-function}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=250pt]{chapters/part-3/figures/qplot_demo.png}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=250pt]{chapters/part-3/figures/mortgage_demo1.png}
	\caption{A demonstration of \texttt{qplot} on mortgage price data frame.} \label{ch:r1:fig:mortgage_demo1}
\end{figure}

\begin{figure}[!htb]
	\centering
	\includegraphics[width=250pt]{chapters/part-3/figures/mortgage_demo2.png}
...
Function \verb|ggplot()| is the main data visualization tool in \verb|ggolot2| package. It provides very flexible features for data plotting. More details are given in the section. Before digging into the details of \verb|ggolot2|, grammar of graphics is introduced first. Later it will become clear that \verb|ggolot2| is practices grammar of graphics.

\subsection{Grammar of Graphics}

As proposed by Leland Wilkinson's \mync{grammar of graphics}, a chart shall contain multiple independent and reusable layers including
\begin{itemize}
...
Notice that the first 3 layers, i.e. data, aesthetics and geometries layers are essential layers, while the rest layers are optional. A graph shall contain at least the first 3 layers for the minimum visualization.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=250pt]{chapters/part-3/figures/chart_layer.png}
	\caption{Multiple layers in chart design.} \label{ch:r1:fig:chart_layer}
\end{figure}

\subsection{Data, Aesthetics and Geometries Layers}

Function \verb|ggplot()| is a very good practice of implementing the grammar of graphics. The basic syntax of \verb|ggplot()|, just for quick demonstration purpose, is given below. In the basic syntax, only data, aesthetics and geometries layers are concerned.
\begin{lstlisting}
p <- ggplot(data=<df>, 
            aes(x=<column>, y=<column>, color=<column>, size=<column>)
     ) # data and aesthetics
p <- p + geom_point() # geometries
\end{lstlisting}
where \verb|aes()| is used to build mappings in the aesthetics.

An interesting fact when using \verb|ggplot()| is that, when adding a layer to the chat, the layer is literally added to \verb|ggplot()|. For example, in
\begin{lstlisting}
p <- ggplot(data=<df>, aes(x=<column>, y=<column>, color=<column>, size=<column>))
p1 <- p + geom_point() # scatter plot
p2 <- p + geom_line() # line plot
p3 <- p + geom_line() + geom_point() # both scatter and line
p4 <- p + geom_point(aes(size=<another-column>)) # overwrite aesthetics when adding scatter plot
\end{lstlisting}
the added layers are able to inherit the aesthetics settings, if they are not overwritten. And speaking of overwriting, even the x and y axis can be overwritten. The displaying name of the labels can be overwritten by stack \verb|xlab("")| and \verb|ylab("")| into the chart.

Function \verb|ggplot()| provides many choices for geometries. The most commonly used ones are summarized in Table \ref{ch:r1:tab:geoms}. Notice that while some of the \verb|geom_*| functions are purely geometries layers and only reflect what is in the data itself (such as scatter plot and line plot), others may paired with a statistics layer function (such as histogram). When adding the later type of \verb|geom_*| functions, statistics layers are also implemented behind the screen.

\begin{table}[!htb]
  \centering \caption{Commonly used \texttt{ggplot} geometries and statistics layers commands. A full list can be found at \cite{ggplot2025package}.}\label{ch:r1:tab:geoms}
  \begin{tabularx}{\textwidth}{llX}
    \hline
    \textbf{Geom} & \textbf{Statistics} & \textbf{Description} \\ \hline
	\texttt{geom\_point()} & --- & Scatter plot. \\
	\texttt{geom\_line()} & --- & Line plot. \\
	\texttt{geom\_segment()} & --- & Drawing a segment. \\
	\texttt{geom\_polygon()} & --- & Polygons plot. \\
	\texttt{geom\_contour()} & --- & Contour plot. \\
	\texttt{geom\_text()} & --- & Text. \\ \hline
	\texttt{geom\_histogram()} & \texttt{stat\_bin()} & Histogram. \\
	\texttt{geom\_density()} & \texttt{stat\_density()} & Kernel density estimate of a distribution. \\
	\texttt{geom\_bar()} & \texttt{stat\_count()} & Bar plot (occurrence plot). \\
	\texttt{geom\_smooth()} & \texttt{stat\_smooth()} & Linear regression model. \\
	\texttt{geom\_boxplot()} & \texttt{stat\_boxplot()} & Distribution with quartiles and outliers. \\
	\texttt{geom\_violin()} & \texttt{stat\_ydensity()} & Combination of density estimation and box plot. \\
	\hline
    \end{tabularx}
\end{table}
...
\begin{figure}
	\centering
	\includegraphics[width=250pt]{chapters/part-3/figures/mortgage_barplot_demo.png}
...
\begin{figure}
	\centering
	\includegraphics[width=250pt]{chapters/part-3/figures/geom_smooth_demo.png}
...
\begin{figure}
	\centering
	\includegraphics[width=250pt]{chapters/part-3/figures/mortgage_histogram_demo.png}
...
\begin{figure}
	\centering
	\includegraphics[width=250pt]{chapters/part-3/figures/mortgage_histogram_demo_f1.png}
...
\begin{figure}
	\centering
	\includegraphics[width=250pt]{chapters/part-3/figures/mortgage_histogram_demo_f2.png}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=250pt]{chapters/part-3/figures/coordinate_demo1.png}
	\caption{Add coordinates layer using \texttt{xlim()} and \texttt{ylim()}.} \label{ch:r1:fig:coordinate_demo1}
\end{figure}

\begin{figure}[!htb]
	\centering
	\includegraphics[width=250pt]{chapters/part-3/figures/coordinate_demo2.png}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=250pt]{chapters/part-3/figures/mortgage_histogram_theme.png}
