...
This and the next chapters introduce R language, a widely used programming language for data mining and visualization. 

This chapter focuses on the fundamental setup and basic operations such as the installation of R and RStudio, basic data types, vector and data frame manipulation, and basic visualization methods. Chapter \ref{ch:r2} introduces advanced data analysis skills and tools.

Comparing with other programming languages such as Python, R is simpler when comes to data visualization for statistics purposes and more efficient when performing statistics operations on large datasets.

\section{R and RStudio Installation} \label{ch:r1:sec:rinstallation}

\mync{R language} is a programming language for statistical computing and visualization. It is widely used among statisticians and data miners for developing statistical software and carrying out data analysis. R is free and can be downloaded from \cite{r}, from which more details about R can also be found.

\mync{RStudio}, also known as \mync{Posit}, is an IDE widely used for R programming and testing. RStudio IDE has an open-source and free-of-charge community version for personal use. It can be downloaded from \cite{rstudio}.
...
R uses packages, also known as libraries, to expand its capability. R packages, both built-in and third-party, provide powerful features for data analysis and visualization. Some packages may also come with demonstrative sample data frames. The packages can be published and shared online. CRAN is by far the most popular platform to store and share R packages.
...
One can also use the functions and data frames in a package without loading it. In that scenario, use prefix \verb|<package>::<function>|, \verb|<package>::<dataframe>| to call the functions and data frames. This is inconvenient if the functions or data frames are to be used frequently.
...
To retrieve the elements in a vector, use either \verb|<vector>[<index>]| which returns a sub-vector that contains the element, or \verb|<vector>[[<index>]]| which returns the element itself. If the elements have names, \verb|<vector>[<name>]| or \verb|<vector>[[<name>]]| respectively as alternative approaches.
...
Alternatively, use \verb|<list>$<name>| to access the content. Following the above example,
\begin{lstlisting}
> print(list2$name)
[1] "lu"
