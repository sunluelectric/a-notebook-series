...
RStudio provides a graphical interface to manage packages as shown in Fig. \ref{ch:r1:fig:rstudio_package_management}. A package can be loaded or unloaded simply by checking or unchecking the corresponding box.
\begin{figure}[!htb]
\centering
...
Variable names are used as identifiers to variables. Legitimate variable names are a combination of letters, numbers, periods (\verb|.|) and underscores (\verb|_|). The name cannot start with a number of the underscore, but can start with the period. A name cannot contain dash (\verb|-|), space or special characters such as \verb|@| or \verb|$|.

It is worth emphasizing the use of period (\verb|.|) in R and in other languages. Many languages such as Python and MATLAB uses the period to indicate fields in a structure. For example, there might be a structure called ``\verb|student|'', and \verb|student.name|, \verb|student.age| are the fields under \verb|student|. However, this is not the case with R. In R, \verb|student|, \verb|student.name| and \verb|student.age| are treated as different variables without logical connections.

For R, it uses \verb|<variable>$<field>| to indicate fields of a structure. More details will be given in later sections when list and data frame are introduced.


\subsection{Different Types of Scalars}
...
Notice that \verb|NA| will be returned if the index used to access the element in a vector exceeds its length, or if the name used to access the element in a vector is not found. This is different from most programming languages, which will usually raise an exception or an error. To remove the name of a vector, simply assign \verb|NULL| to \verb|names(<vector>)|. 
...
R provides flexible and powerful data visualization tools, many of which more advanced than the ones to be introduced in this section. This section introduces a simple matrix visualization function called \verb|matplot()|, which plots the columns of a matrix against each other.
...
As introduced earlier, there are two ways to index elements in a vector, matrix or list, namely \verb|[]| and \verb|[[]]|. While in the context of vectors and matrices \verb|[]| is almost always used, in the context of list \verb|[]| and \verb|[[]]| both have purposes and they can make a big difference. And finally, \verb|<list>$<name>| is simply a shorthand for \verb|<list>[[<name>]]|. Examples are given below.
\begin{lstlisting}
> print(list2$name)
[1] "lu"
...
\subsection{Conditional and Loop Statements}
