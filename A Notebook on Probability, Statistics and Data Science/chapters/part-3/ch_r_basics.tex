...
\mync{RStudio}, also known as \mync{Posit}, is an IDE widely used for R programming and testing. RStudio IDE has an open-source and free-of-charge community version for personal use. It can be downloaded from \cite{rstudio}. Notice that RStudio is not compulsory and there are other IDEs that supports R script programming such as VSCode and Vim. With that been said, most of the R scripts in this notebook are programmed and tested with RStudio.

\subsection{Installation}

Download R and RStudio from the aforementioned web sites, and install them following the instructions on their official websites. It is required to install R before installing RStudio.

\subsection{R Packages Management} \label{ch:r1:sec:rpackage}

R uses packages, also known as libraries, to expand its capability. R packages, both built-in and third-party, provide powerful features for data analysis and visualization. Some packages may come with demonstrative sample data frames. Packages can be published and shared online. CRAN is by far the most popular platform to store and share R packages.
...
library(pacman)
\end{lstlisting}
After loading a package, the data frames and functions defined in that package can be used normally. It is possible to use the functions and data frames in a package without loading it, in which case use prefix \verb|<package>::<function>|, \verb|<package>::<dataframe>|. This is inconvenient if the functions or data frames are to be used frequently.
...
RStudio provides a graphical interface to manage packages as shown in Fig. \ref{ch:r1:fig:rstudio_package_management}. A package can be loaded or unloaded simply by select and deselect the package.
\begin{figure}
\centering
...
\section{Basic Syntax and Data Types} \label{ch:r1:sec:rprogramming}

This section introduces the basic syntax and commonly used data types of R.
...
> print(c)
[1] 1 2 3 4 5 6
> d <- append(c, c(100, 200, 300), 1) # insert a vector after the Xth element
> print(d)
...
Notice that \verb|NA| will be returned if the index used to access the element in a vector exceeds its length, or if the name used to access the element in a vector is not found. This is different from most programming languages that will return an error.

To remove the name of a vector, simply assign \verb|NULL| to \verb|names(<vector>)|. To loop over an vector, simply use
\begin{lstlisting}
...
In the context of unnamed numeric vectors and matrices, \verb|[[]]| is rarely used, and \verb|[]| works almost all the time. However, when comes to other data structures like list, etc., it is important to distinguish \verb|[]| and \verb|[[]]|, and choose the correct one depending on the purpose.
...
[1] TRUE
\end{lstlisting}
It is possible to deliberately ask R to not drop the matrix dimensions. This can be done as follows. By doing this, the names assigned to columns and rows preserve.
\begin{lstlisting}
> A <- matrix(1:9, 3, 3)
...
As introduced earlier, there are two ways to index elements in a vector, matrix or list, namely \verb|[]| and \verb|[[]]|. While in the context of vectors and matrices \verb|[]| is almost always used, in the context of list \verb|[]| and \verb|[[]]| both have purposes and they can make a big difference. And finally, \verb|<list>$<name>| is simply a shorthand for \verb|<list>[[<name>]]|. Examples are given below.Alternatively, use \verb|<list>$<name>| to access the content. Following the above example,
\begin{lstlisting}
> print(list2$name)
[1] "lu"
...
One of the most common sources of data is a CSV file. R provides convenient functions to read data from CSV files into data frames. Use the following commands to import data from a CSV file into a data frame.
