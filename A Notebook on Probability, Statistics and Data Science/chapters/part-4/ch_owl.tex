...
RDF/RDFS is able to process simple logic reasoning, but cannot handle description logic. Web ontology language is proposed to enable description logic in the semantic web.

\section{RDF/RDFS Limitations} \label{subsec:rdfrdfslimitations}

Limitations of RDF/RDFS are illustrated with examples.
...
\mync{Web ontology language}[OWL] essentially introduces DL inference to the semantic web, making it more expressive and capable of complicated reasoning. With that in mind, OWL provides additional features to enforce and enhance the schema of an RDF model, thus addressing the problems mentioned in Section \ref{subsec:rdfrdfslimitations}. It allows adding relations and property constraints to the RDF model. The following summarizes the main features introduced by OWL.
\begin{itemize}
...
\subsection{Equivalent and Different Individuals}
