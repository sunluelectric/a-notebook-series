...
Ontology inspires computer scientists on how information can be stored and exchanged efficiently on the internet, which leads to semantic web, an internet-based knowledge base schema. 

The semantic web, together with other technologies, defines Web 3.0, a new-generation internet framework. Ontology, logic reasoning and semantic web are introduced in this chapter.

\section{Web of Data}

The internet is a technology that enables information exchange among machines. With the power of the internet, users can obtain information from remote servers, databases, or knowledge bases. Note that there are subtle differences between ``internet'' and ``Internet''. The lowercase ``internet'' refers to the technology that bridges machines to form a network of any size, while the uppercase ``Internet'' refers to the specific internet that links the entire world together. In this part of the notebook, however, they are used interchangeably.
...
One-way data transmission in Web 1.0 cannot meet the expectations of users who want to share their information with other users on the internet. Thanks to advances in information science and communication, under the \mync{Web 2.0} framework users can interact with the internet bidirectionally. A user can search and filter data from the servers and even upload his own data and share it with others. The internet became far more powerful in terms of information exchange. The source of information does not necessarily come from the authorities. The users are producers and consumers of information at the same time. Examples of Web 2.0 implementations include Blog, Twitter, YouTube and Weibo.

Web 2.0 is extremely popular and successful even to date. Yet, under the background of industry 4.0 where \myabb{internet-of-things}{IoT} and big data drive almost everything, there are still limitations and downsides to Web 2.0 that we would wish to improve.
...
The goal of \mync{Web 3.0} is to allow more efficient information retrieval and sharing using the internet. 

We have discussed the limitations of Web 2.0 where the data retrieval can be too noisy. Ideally, the search engine should truly understand the user's demand and return only the most relevant information summarized in a clear manner. If there is no readily available response on the internet, the search engine should derive the response based on existing information, i.e., it should be capable of simple reasoning. 

For this purpose, there are at least the following two development trends.
\begin{itemize}
  \item Let the LLM-based AI interpret all the knowledge. The LLM should be smart enough to precisely capture what the user wants and get back to him with useful and accurate information. This leads to chatbot, copilot, \myabb{resource augmented generation}{RAG} and agentic AI.
  \item Create a powerful and flexible NoSQL database. Make the information in the database readable for both humans and machines, and somehow integrate the semantics of the contents into the database. This leads to \mync{semantic web}, a graph-based knowledge base schema.
\end{itemize}
...
\begin{itemize}
	\item LLM-based solution
	\begin{itemize}
		\item Running LLM online is time and computational load wise expensive, and often requires large communication bandwidth. 
		\item It is hard to scale up the knowledge base efficiently, even with RAG.
		\item LLM lacks the ability of reasoning in a systematic manner, and the ``smartness'' of LLM does not come from logic reasoning and may lead to hallucination.
		\item LLM base knowledge cannot be expanded or migrated easily, as it is in the form of regression coefficients not accessible by human or another machine.
	\end{itemize}
	
	\item Semantic web
	\begin{itemize}
		\item In design phase, just like any other databases, it is challenging to design the structure of the semantic web to the optimum, and it requires joint effort from domain expert and knowledge engineer.
		\item In implementation phase, there is a steep learning curve for database manipulation.
		\item Semantic web alone returns JSON objects and it is not good at summarizing and formatting the response in a human-friendly manner.
	\end{itemize}
\end{itemize}

Web 3.0 schema focuses on semantic web. The word ``semantic'' in this context specifically refers to the relations of objects and properties that can be used for machine-driven descriptive reasoning. Semantic web is so important that it is often considered synonymous with Web 3.0, although the broader vision of Web 3.0 includes other features as well.

\begin{mdframed}
\noindent \textbf{Web 3.0 Features Besides Semantic Web}

Another important feature of Web 3.0 is distributed storage of information. 

Distributed storage has both advantages and disadvantages. On one hand, it provides higher resilience against data loss. On the other hand, it adds challenges to the searching and collecting of information when performing query. Technologies like \myabb{InterPlanetary File System}{IPFS}, blockchain, and distributed databases are used to address these challenges.

\end{mdframed}

This part of the notebook discusses the technologies used in semantic web such as \myabb{resource description framework}{RDF} model, \myabb{RDF schema}{RDFS} and \myabb{web ontology language}{OWL}. We will see how we can use semantic web to collect and organize information, and create a knowledge base for both humans, machines, and AI.
...
\begin{itemize}
	...
	\begin{itemize}
		\item \myabb{Uniform resource identifier}{URI} that identifies and traces an information source. More about URI is introduced in Section \ref{sec:uri}.
	...
	\begin{itemize}
		...
		\item \myabb{SPARQL Protocol and RDF Query Language}{SPARQL} which is the recommended language for semantic web query (SPARQL 1.0) and manipulation (SPARQL 1.1).
	\end{itemize}
	\item Logic
	\begin{itemize}
...
RDF, RDFS, OWL and SPARQL are introduced formally in later Section \ref{sec:rdf}, Section \ref{sec:rdfs}, Chapter \ref{ch:owl} and Section \ref{sec:sparql} respectively. Below are brief introductions to these concepts just as a heads up.

The knowledge and semantics are stored using the RDF/RDFS model. The RDF/RDFS is often further powered by OWL. RDF serves as the foundation with the basic structure. RDFS expands the capability of RDF by introducing new vocabularies such as ``class'', ``subclass'', ``property''. And finally OWL enables a set of ways to define complex ontologies and create rigorous and complex semantics. 

As an analogy, think of RDF as the paper and pencil, RDFS the 24-color crayon set, and OWL the drawing skills. Together they make a sophisticated and richly structured picture.

Many syntax such as RDF/XML and JSON-LD can be used as markup languages to describe RDF/RDFS/OWL used in a semantic web. They describe the same contents and logic, but just in different languages. There is no globally optimal syntax. More details are introduced in later chapters.

There are several query and manipulating languages for semantic web, and \myabb{SPARQL Protocol and RDF Query Language}{SPARQL} is probably the most adopted one. It allows users to search, retrieve, and modify information stored in the RDF model. The syntax of SPARQL is designed to look similar to SQL, and many commands in SPARQL have counterparts in SQL, such as \verb|SELECT|, \verb|WHERE|, \verb|GROUP BY|, and \verb|ORDER BY|. However, the backend technologies of a semantic web engine, known as \mync{triplestore} or \mync{RDF management system}, differ largely from that of a relational database management system. For example, triplestore relies heavily on graph theory and graph pattern mapping when searching through a semantic web.
...
SPARQL can be used to query the above semantic web. An example is given below.
...
Applications cannot expect the end user to write SPARQL. Commercialized semantic web applications such as WolframAlpha (\textit{wolframalpha.com}) often provide a search bar which looks similar but works entirely differently with a conventional key-word based search bar. The input to the search bar is converted into SPARQL query by a natural language processing engine (such as an LLM) and then executed with the triplestore.
...
\mync{Ontology} is the philosophical study of the nature of being, existence, or reality. It concerns ``what is everything'' and ``how to define everything'' in the context of inductive and deductive reasoning. It discusses how humans abstract and preserve knowledge. Ontology has very rich meanings from both philosophy and semantic web perspectives. 
...
Ideally in the vision of \myabb{World Wide Web Consortium}{W3C}, all ontology models should be linked together using URIs and form a global model just like the global Internet. In this ultimate global model, ontology is divided into layers. The higher the layer, the more general the knowledge. The lower the layer, the more specific the knowledge within a particular domain, application or task. 

This is illustrated by an example in Fig. \ref{fig:ontologylevel} on spacecraft control. Lower-layer ontology can inherit and modify knowledge from the upper-layer ontology. In practice, there might not be a clear boundary between two adjacent layers.
...
\mync{Logic} refers to the systematic way of deriving new information from existing knowledge. The term ``formal logic'' rigorously defines logic reasoning methods and procedures to automate logic inference by machines.

Notice that logic and logical expression are by themselves very complex and rich concepts. It is not possible to include all the details about them in the scope of this notebook. Only a scratch is given in this section.
...
The Python interpreter is purely syntax-driven, and it follows the provided instructions without any broader comprehension or anticipation of what the code is doing. In the context of this example, the interpreter does not recognize that it is generating the famous Fibonacci sequence. It simply follows the rules of the script, calculating and outputting each number in the series as instructed.
...
\subsection{Logic Frameworks} \label{subsec:knowledgeandlogic}

Different logic frameworks have different levels of complexity and capabilities of expressing semantics. Commonly seen logic frameworks are briefly introduced as follows.

\mync{Propositional logic} is the fundamental of logic reasoning. In propositional logic, knowledge is represented by either simple facts which are known as propositions, or facts connected with ``AND'', ``OR'', ``NOT'', ``IF ... THEN ...'', ``IF AND ONLY IF'' which are known as compound propositions.

\mync{First-order logic}[FOL] is the most commonly used logic as of today. In FOL, quantifiers and logic connectives are introduced, including universal quantification $\forall$, existential quantification $\exists$, conjunction $\land$, disjunction $\lor$ and negation $\neg$. A formal way of representing logic expressions are defined. For example, using the following to represent ``all humans are mortal''
\begin{eqnarray}
	\forall x \left(\textup{Human}(x) \rightarrow \textup{Mortal}(x)\right) \nonumber
\end{eqnarray}
where $\textup{CLASS}(x)$ is equivalent of the proposition ``$x$ belongs to CLASS'', which can be either true or false. The above proposition says ``for any instance, if it is true that the instance belongs to human, then it must be true that the instance belongs to mortal''.

\mync{Description logic}[DL] is a subset of FOL. Derivation of DL from FOL is not under the scope of this notebook. Some key features of DL are summarized as follows.
\begin{itemize}
...
\end{itemize}
Semantic web uses OWL to implement DL on computers. More details are given in Chapter \ref{ch:owl}.

\begin{mdframed}
	
\noindent \textbf{Undecidability in FOL}

We would surely want to introduce highly expressive and complex formalisms to the existing RDF/RDFS model. This motivates OWL, which enables DL in semantic web. More details of OWL is introduced later in Chapter \ref{ch:owl}. 

Notice that DL instead of FOL is widely used in semantic web due to the fact that FOL is undecidable.

A logic is decidable if there is an algorithm that can determine whether any given statement in the logic is true or false. If no such algorithm exists for a logic, it is undecidable. G$\ddot{o}$del's Incompleteness Theorems indicates that in any sufficiently powerful logical system such as FOL, there are statements that are true but cannot be proven within the system. FOL, therefore, is undecidable. Consequently, an FOL reasoning algorithm may not terminate in finite time, and this can cause issues in many applications.

The proof of G$\ddot{o}$del's Incompleteness Theorems can be found elsewhere and is not given in this notebook.
...
\subsection{Logical Expression} \label{sec:logical_expression}

A logic is defined by 
\begin{eqnarray}
	L &:=& (S, \models) \label{eq:logic}
\end{eqnarray}
where $S$ is the set that contains all the statements of our interests, and $\models$ the entailment relation
\begin{eqnarray}
	\models &=& \models^1 \bigcup \models^2 \label{eq:entailment_relation} \\
...
\end{eqnarray}
Note that the entailment relation \eqref{eq:entailment_relation} is given in the form of set as they are mathematically equivalent. 

A logic statement is meaningful only if its logic interpretation $I$ is clearly articulated. \mync{Logic interpretation} is a formal construct that defines the meaning of a symbol in a formal language. For example, in the ontology of people, an interpretation can map an instance symbol, such as ``Alice'', to an actual person in the real world, and class symbol ``Person'', to the concept of a human being, etc. Logic statement, formally  known as \mync{logic formula}, is a statement formulated by string of symbols from a formal language. It is an assertion trying to represent some fact. For example, a formula can be ``Alice hasSibling Bob''. 

The true or false of the formula depend not only by the formula itself, but also by the interpretation. In the earlier example ``Alice hasSibling Bob'', if ``Alice'' and ``Bob'' are indeed mapped to two siblings, and the relation ``hasSibling'' is as it literally represents, then it is true. Consider another example, ``10 is greater than 5''. While this formula may sound universally true, it actually depends on the interpretation. If ``10'' and ``5'' are interpreted as numerical quantities and ``is greater than'' as the standard numerical greater-than relation, then it is true. However, if ``10'' and ``5'' are interpreted as amounts of debt and ``is greater than'' as ``richer'' (with less debt being richer), then it would be false under this interpretation.

The interpretations that make the formula true form \mync{the model of the formula} denoted by set $I(F)$ or $I \models F$, which read as ``I models F'' or ``I satisfies F''. 

\begin{mdframed}
\noindent \textbf{Does logic tell whether a formula is true or false?}

No. Logic given by \eqref{eq:logic} does not tell specifically if a formula is true or false. It only specifies a set of formulas and their true false relationships (known as inference) in the form of ``if the formulas in set $\Phi$ is true, then formulas in set $\Psi$ must also be true'' which is denoted by $\Phi \models \Psi$.

However, it does not tell whether formulas in $\Phi$ are true or false in the first place. As a result, we do not know the true false of a formula from the logic alone. The true and false of a formula depends on the interpretation of the formula.

For example, we may have a logic that says
\begin{eqnarray}
	\forall X: \textup{Child}(X) \rightarrow \textup{lovesIcecream}(X) \nonumber
\end{eqnarray}
which translates to ``for all instances denoted by $X$, if $X$ interpreted as Child holds true, then $X$ interpreted as lovesIcecream must also be true'', or in other words, ``all children love ice cream''. However, this does not tell you whether $\textup{Child}(\textup{Alice})$ is true or false, nor $\textup{lovesIcecream}(\textup{Alice})$. That depends on the interpretation of Alice. We do know from the logic that if Alice is a child, Alice must love ice cream.

\end{mdframed}

Multiple formulas can be grouped into a \mync{theory} often denoted by $T$. The formulas in a theory are often treated as axioms, meaning that they are assumed true as a starting point. With a theory and the logic, logic reasoning can be performed to prove true or false of a new formula not in the theory. A theory can be treated as a knowledge base.

\subsection{Logical Equivalence}

Let $\Phi, \Psi \subseteq S$. If $\Phi \models \Psi$ and $\Psi \models \Phi$, $\Phi \equiv \Psi$ and they are logically equivalent.

In a special case where $\Phi$ and $\Psi$ each contains only one formula given by $F$ and $G$ respectively, the notations that applies to $\Phi$ and $\Psi$ also applies to $F$ and $G$. For example, $F\models G$ means that under the same interpretation if $F$ is true, $G$ must also be true. If $F \models G$ and $G \models F$, $F$ and $G$ are logically equivalent denoted by $F\equiv G$.
...
\mync{Logical reasoning} is about proving true or false of a formula $F$ given a theory $T$ and a logic $L$. Consider the following example. Given logic
\begin{eqnarray}
\forall X: \textup{Child}(X) \rightarrow \textup{lovesIcecream}(X) \nonumber
\end{eqnarray}
and theory $\textup{Child}(\textup{Alice})$, it can be proved that $\textup{lovesIcecream}(\textup{Alice})$. 

In a complex logic system, the proof may not be unique, and sometimes it is difficult to find one. 

It is possible that due to the lack of information, a formula cannot be proved true or false. Practically a knowledge base handles this situation differently depending on the setup. When \mync{open world assumption}[OWA] is made, the knowledge base considers itself as underdevelopment, and it is open-minded to unknowns. While when \mync{closed world assumption}[CWA] is made, the knowledge base considers itself as developed, and unknown means nonexistence. 

Consider the following example. A knowledge base of a family tree is asked ``are Alice's children all males?'', and in the database there are two children of Alice, both of which are male. Under OWA, the inference would return ``maybe'', as it does now know whether Alice has more children not yet registered in the database. While in CWA, the inference would return ``yes'', as it checks both children to be male, and it assumes they would be all the children Alice has. However, when Alice does have female child registered, both OWA and CWA inferences would return ``no'', because the female child contradicts the assertion regardless of the completeness of the database.

\mync{Law of contradiction} is widely used in logic reasoning. For example, to prove $T\models F$, just find contradictions in $\{\neg F, T\}$. A commonly used way of looking for contradictions in $\{\neg F, T\}$ is to resolve the formula into something more structured, for example to one of its canonical forms. Commonly used canonical forms for contradiction check are clausal normal form and disjunctive normal form.
