...
\myabb{Large language model}{LLM} is one of the many solutions to \myabb{natural language processing}{NLP} as part of sequential data processing and generative \myabb{artificial intelligence}{AI}. In the past few years we have seen revolutionary development in LLM, and hence it forms a dedicated part in the notebook. With the introduction of multimodal LLM, the capability of LLM expands beyond language processing towards image and even video processing. 

Modern LLM is built on top of Transformer, an AI framework proposed in the landmark paper ``Attention Is All You Need'' \cite{vaswani2017attention}. Before formally introducing LLM in the following chapters, Transformer is introduced in this chapter.
...
\myabb{Recurrent neural network}{RNN} has been introduced in earlier Chapter \ref{ch:rnn}. A brief review is given in this section, focusing on its shortages in NLP.

\subsection{A Brief Review of RNN}

In a conventional dense ANN, the correlation of consecutive inputs cannot be captured efficiently. A dense ANN would treat all the inputs equally, without structurally addressing the sequence and distance between words. Large number of nodes, layers and training data are often required to compensate for the lack of structural information in the dense ANN framework to achieve a well performed system. In addition, dense ANN requires fixed input length. Although it is possible to use ``NULL'' as placeholders for unused characters, the upper limit of length is bounded.

Compared with dense \myabb{artificial neural network}{ANN}, RNN is more efficient in building up correlations between consecutive inputs, and hence it becomes a powerful framework for sequential data processing. RNN tries to tackle the above issues by implementing a ``pre-processing'' stage, where the correlation of the sequential data is first abstracted. The correlation information is then amended as additional inputs to the followed up dense ANN.
