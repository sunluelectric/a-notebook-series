...
As far as this notebook concerns, almost all the codes are in Python. There are many environment and package managers for Python. Two popular ones, \verb|conda| and \verb|uv|, are introduced in this appendix chapter.
...
Miniconda is a lightweight version of Anaconda distribution. The former contains only \verb|conda|, \verb|python| and a small number of other packages, while the latter contains more than three hundred packages. Some of them are proprietary to Anaconda and may require a license for use in production environments.
...
where additional \verb|-f| followed by the list of packages in TXT or YAML file (conventionally, \verb|environment.yml|) can be used to create an environment and install required packages. In this case, \verb|conda| automatically checks and configures the machine setup and handles package dependencies. 
...
In addition to \verb|uv add|, \verb|uv| provides the following command
\begin{lstlisting}
