...
Python program can connect to the model either via HTTP calls, or with Python \verb|ollama| package. For the sake of convenience, only the latter is introduced.
...
While Ollama provides a simple way to deploy local LLMs, a user can also download an open-source LLM elsewhere (for example, from Hugging Face), and study, fine-tune and deploy it by himself. More details will be introduced in later sections.
...
Many LLM service providers, such as OpenAI, allows the user to interact with their cloud-based LLMs using APIs. Each API call introduces a cost which often depends on the model it uses and the token the questions and responses consume. If the user has decided to use a commercialized LLM model via its API key, he needs to register an account with the LLM provider, such as OpenAI, and create a new API key, and added it to the project as an environmental variable. 
...
\subsection{A Review of Frontier Models}

\subsection{Frontier Model Fine-Tuning}
