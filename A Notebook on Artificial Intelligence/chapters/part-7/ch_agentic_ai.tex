...
Once the state is defined, a graph can be declared as follows.
\begin{lstlisting}
class State(TypedDict):
    <defined earlier>
...
\end{lstlisting}
And all the nodes and edges what will be added to this graph later will use the defined state in the data pipeline.
...
Edges can be either deterministic or selective. For selective edges, the upstream node decides which edge to follow.
...
\end{lstlisting}
Here, \texttt{START} and \texttt{END} represent the default entry and exit points of the graph. At the start point, the agentic AI system receives input from the user, and at the end point, it returns a response. The entire execution of the graph is referred to as a \mync{super step}.
...
LangGraph allows packaging a function into a tool conveniently. An example is given below.
