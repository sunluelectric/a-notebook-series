...
This chapter serves as an overview of \mync{large language model}[LLM], a popular and successful NLP practice. Not only is LLM able to process human languages, but also can it generate new contents based on human language written requirements. The introduction of multimodal LLM further allows LLM to understand and generate not just writings but also pictures and videos. The theory and technology behind LLM are briefly discussed.

Nowadays, it is possible for a regular user to leverage on commercialized models to develop his own applications. Given a powerful machine, a user can also deploy LLMs locally. Open-source LLMs are available so that the user does not need to train a model from scratch. The deployment of LLMs and the interaction with them are introduced in consecutive chapters. 
