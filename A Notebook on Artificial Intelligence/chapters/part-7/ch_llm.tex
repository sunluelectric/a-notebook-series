\chapter{Large Language Model (Part 1: Theory)} \label{ch:llmtheory}

This chapter serves as an overview of \mync{large language model}[LLM], a popular and successful NLP practice, from theory and technical perspective. LLM can not only process human languages but also generate new contents based on human language written requirements. The introduction of multimodal LLM further allows LLM to understand and generate not just writings but also pictures and videos. The theory and technology behind LLM are briefly discussed.
...
This section reviews existing NLP methods and their limitations, and discusses the gap LLM bridges.
...
With the introduction of ANN, in particular RNN, \mync{Neural Language Models}[NLM] became popular. RNN-based NLM builds the word prediction function conditioned on the aggregated context features abstracted and passed recurrently from current and previous input sequence.
...
This section introduces commonly seen frontier LLM models as of this writing. Some of the most famous ones are listed below.

\begin{itemize}
  \item Open-source
  \begin{itemize}
    \item LLaMA, and its variations (Meta)
    \item Gemma (Google)
    \item Qwen (Alibaba Cloud)
    \item Deepseek-R1 (Deepseek)
    \item Phi (Microsoft)
  \end{itemize}
  \item Closed-source
  \begin{itemize}
...
Each of the models listed above has its own strengths and weaknesses. Since the models are continuously evolving, this notebook will not address or compare their shortages. Instead, only general features that are shared among all LLMs are introduced in this section.

The user can interact with these models using the interfaces they provide such as CLIs and APIs. The interfaces are also introduced.
...
\section{LLM Development Workflow}
...
Data corpus is available online for LLM training. Many libraries such as PyTorch compatible Python packages have been developed to automate the training procedures and to convenient the users. Many LLMs, both open-source and closed-source, are available on the market. A summary is given in Table \ref{ch:llm:tab:existingmodel}. Notice that the table only covers a small portion of existing models in the market.
...
\section{A Brief Review of Multimodal LLM}
