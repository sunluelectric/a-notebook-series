...
To emulate the utility of a state using \eqref{eq:vfapproximation}, we need to choose the feature of the state $\phi(s)$. This is often experience based, where $\phi(s)$ can contain the original $s$ together with nonlinear components derived from components of $s$ that may contribute to the utility and reward of the state, i.e., anything that may correlatively relate to ``how well the system is performing''. With the massive synthetic data generated from the model, the user can choose a large set of features without necessarily worrying about over-fitting.
