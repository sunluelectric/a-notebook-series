...
To formulate a decision-making problem mathematically, the first step is to define the \mync{utility} (often in the form of a function) that quantifies the value of an action or a system state. Decision making is essentially the process of maximizing utility. This section introduces the concept of utility and its formulation, and how utility is used in decision making with AI.
...
When an AI agent decides to perform an action among all the actions it can take, the system transitions from one state to another. Let the \mync{transition model} be denoted by $P(s|a,e)$, where $e$ represents the evidence or observation of the system, incorporating the agent’s awareness of its current origin state, $a$ the action taken, and $s$ the destination state. Notice that $P(s|a,e)$ is given in the form of probability, which captures uncertainty in the system.
...
MDP provides a framework to tackle such problems. Through MDP, the agent can learn via trial and error to calibrate states and actions utilities (known as \mync{value functions} in the context of MDP) and the transition model. There will be failures in which the agent falls into traps and successes in which it finds the goals. Through repeated trials and reinforcement learning, it can eventually determine what action to take at each position to avoid traps and reach goals as efficiently as possible.
...
\begin{mdframed}
	...
	The following examples illustrate how to decide what information should be included in the state. Consider a robot navigating a maze. It knows its current position, represented by coordinates, but it does not know the locations of walls. 
	...
	\begin{itemize}
		...
		\item Scenario 2: The robot needs to exploring as many unique locations as possible within a time horizon.
...
\begin{itemize}
	...
	In some problems, the agent gains a reward or penalty upon staying or reaching a particular state. The previous state or the action taken to reach that state is irrelevant.  
