...
It is theoretically possible to explicitly program a rule-based system that can mimic human behavior, but the realization will be too expensive and very likely not robust enough. Nowadays it has reached a common sense that machine learning should be used to build AI.

\subsection{Machine Learning}

\mync{Machine learning} refers to the field of study that gives computers the ability to learn (from the data) without being explicitly programmed to do so. Compared with a explicitly programmed AI, machine learning is often less labor insensitive and more robust.

Depending on the labeling of samples, there are at least the following $3$ types of machine learning problems.
...
There are broadly divided two types of supervised learning problem, depending on the format of output $y$. If $y$ is a continuous value, the supervise learning problem is also known as a \mync{regression} problem which tries to map $y = f(x)$ with a function. If $y$ is a discrete value taken from a finite set, the supervised learning problem is also known as a \mync{classification} problem which tries to find the most likely label for the corresponding input. 

Example: on a house price guide website, given the size and location of a house, estimate its price on the market (regression); given a picture of an animal, classify what animal it is (classification).

\vspace{0.1in}
\noindent \textbf{Reinforcement Learning}
\vspace{0.1in}

In \mync{reinforcement learning}, a sample is given in the form of $(x, a, r)$ where $x$ is the state of the system, $a$ (optional) an action that the system takes, and $r$ the reward. At any state, the system can take an action, transforms to a new state, and meantime gains some rewards. The purpose of reinforcement learning is to build a machine learning system that determines the optimal actions to take that maximizes the utility of the system. The utility often includes not only the immediate reward, but also future foreseeable benefits. 
...
Machine learning plays an important tole in AI. AI can be constructed in many ways. Imagine a rule-based system with very comprehensive rules. If done correctly, it may mimic a portion of human behavior. However, this is very difficult to achieve in practice due to the complexity in human. It is more often that we use machine learning to achieve AI. 

\subsection{Scope of AI}

It is not easy to draw a universally consistent line on what AI studies because our understanding of intelligence is evolving over time. 
...
\subsection{Research and Development Trends}
