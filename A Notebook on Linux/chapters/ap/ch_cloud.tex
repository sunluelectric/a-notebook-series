...
\myabb{Amazon Web Service}{AWS}, Microsoft Azure, \myabb{Google Cloud Platform}{GCP}, etc., are examples of well-known platforms provided by different cloud service providers. Many cloud services are closely tied to Linux. In this appendix chapter, a general introduction to cloud services is given. 

\myabb{Amazon Web Service}{AWS} is one of the most popular cloud services platforms. AWS services are introduced as examples. Similar services are likely provided by other cloud service providers as well. Therefore, most things about AWS introduced in this appendix chapter should adopt to other cloud services platforms.
...
\begin{table}[!htb]
	...
	\begin{tabularx}{\textwidth}{cccccc}
		\hline
		\multirow{3}{*}{Model} & \multirow{3}{*}{Hardware} & App & APP & APP & \multirow{3}{*}{Examples} \\ 
...
When a user registers an account with AWS for the first time, they get the root account. The user can login to the system with the root account, and create other user accounts and assign roles for them. All services, either created by the root account or by the users emerged from the root account, bill from the root account. 
...
\begin{itemize}
	...
	\item Limit the use of root account to the minimum and follow the principle of least privilege. Create user accounts or groups of different roles for different tasks.
\end{itemize}
It is recommended to enable MFA and other login protection mechanisms for all accounts, both root and regular ones.
...
\begin{itemize}
	...
	A user by itself does not come with any privilege or access to any AWS resource. It simply allows AWS to identify ``who is speaking''. They gain their privilege from the roles assigned to them. When a user is added to a group, they automatically inherit the roles of that group.
...
https://<bucket name>.s3.<region>.amazonaws.com/<file name>
	...
	\item Different storage tiers are available, each with features and cost. More to be introduced later.
	\item Server-side data encryption can be enabled if the user chooses to do so.
\end{itemize}
...
\begin{itemize}
	...
	Data lock can be used to prevent data from being accidentally deleted or modified. AWS S3 uses S3 Object Lock that stores objects using write-once-read-many model. When it is effective, the data, once written, cannot be modified or removed. 
	
	S3 Object Lock can be applied on single object or on the entire bucket. S3 Object Lock can work in governance mode where only specified persons can change the object, or in compliance mode where nobody can change the object within the time frame.
