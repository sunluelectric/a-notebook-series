...
\begin{itemize}
	...
	\item Create user accounts or groups of different roles for different tasks, and while assigning roles follow the principle of least privilege.
\end{itemize}
...
\begin{itemize}
	...
	A user is a virtual representation of a person or an application that needs access to the cloud services. A person user can log into the system with their \mync{access key} to reach the AWS console (default dashboard). A user or an application can log into the system with their \mync{secret access key} from \myabb{Command Line Interface}{CLI} or \myabb{Application Interface}{API}.
	
	A user does not come with any privilege or access to any AWS resource by default. It simply allows AWS to identify ``who is speaking''. They gain their privilege from the roles assigned to them. When a user is added to a group, they automatically inherit the roles of that group.
	
	User account is often permanent, while its role assignment can be either permanent or temporary.
	
	\item Role
	
	A role is a collection of privileges and accesses to specific AWS resources. Different from a user, a role is not about ``who is speaking'' but about ``what they can do''. A role can be assigned to a user, in which case the user gains the privilege and access to the AWS resources specified by the role.
	...
	A policy is a piece of JSON object or similar that represents the contents of roles as well as the rules about which users can claim them.
	...
	\begin{lstlisting}
{
	...
	\end{lstlisting}
	which allows a user to take all actions on all resources.
