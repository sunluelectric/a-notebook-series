...
A \mync{NAT gateway} is the ``router'' of a subnet in a VPC. It is necessary if the devices in the subnet wants Internet access. A NAT gateway translates the private IP addresses in a subnet to a global IP address assigned to the NAT gateway. Notice that NAT gateway is necessary only for those devices in the private network that do not possess public IP addresses. The user is able to deploy a device and assign it with a public IP address. Such a device does not require a NAT gateway.

NAT gateways and devices with public addresses connect to the \mync{Internet Gateway}[IGW] of the VPC to connect to the Internet. The connection from these NAT gateways and devices to the IGW is managed by the route table(s) of the VPC.

The relationship of devices in private subnets, NAT gateway, devices with public addresses and IGW is demonstrated in Fig. \ref{fig:aws_vpc_gateway}. 
...
Notice that \mync{public subnet} in this context refers to the collection of devices in the VPC that is assigned with a public IP address. By this definition, NAT gateway is considered a device in the public subnet. For a device in the private subnet to access the Internet, it must access the Internet via a NAT gateway in the public subnet. Whereas for a device that is already assigned with a public IP address, it can access the Internet without a NAT gateway.

The connectivities from the device in the private network to the NAT gateway in the public network and from the NAT gateway and the device with public IP address to the IGW are managed by route table(s) of the VPC.
...
\subsection{AWS VPC Route Table and Access Control List}
