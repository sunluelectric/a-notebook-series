\chapter{Cloud Computing} \label{apch:cloud}

\myabb{Amazon Web Service}{AWS}, Microsoft Azure, \myabb{Google Cloud Platform}{GCP}, etc., are examples of cloud service providers. Many cloud services are closely tied to Linux. In this appendix chapter, a general introduction to cloud services is given. 

\myabb{Amazon Web Service}{AWS} is one of the most popular cloud services platforms. AWS services are introduced as examples. Similar services are likely provided by other cloud service providers as well. Therefore, most things about AWS introduced in this appendix chapter should similarly apply to other cloud services platforms.

\section{Introduction to Cloud Services}

\mync{Cloud services} refer to a collection of managed cloud-platform-based services including computing, networking, data storing, etc. The most important advantage of cloud-based solutions, comparing with the traditional data-center-based solutions, is that it can be easily scaled up and down with little or no blackout time, allowing customers to pay only for operating expenses but not capital expenses and redundancies. Cloud solutions are famous for high availability, robustness, accessibility and efficiency.
...
\begin{itemize}
	\item \mync{Infrastructure as a Service}[IaaS], where the cloud service provider provides the hardware infrastructure, and the user sets up runtime environment for the application and also deploys the source code.
	\item \mync{Platform as a Service}[PaaS], where the cloud service provider provides the hardware and the runtime environment, and the user deploys application source code.
	\item \mync{Function as a Service}[FaaS], also known as \mync{serverless}, where the cloud service provider provides the hardware and the engines to execute the user scripts, and the user provides the user scripts containing the algorithm or logic. The user also sets up the trigger for the user scripts. The cloud service provider tracks the trigger and execute the user scripts accordingly.
	\item \mync{Software as a Service}[SaaS], where the cloud service provider provides the cloud-based application, and the user only manages the data but not the application program or algorithm.
\end{itemize}
...
\begin{table}[!htb]
	...
	\begin{tabularx}{\textwidth}{cccccc}
		\hline
		\multirow{3}{*}{Model} & \multirow{2}{*}{Hardware} & Runtime & Source & Features & \multirow{2}{*}{Examples} \\ 
		& & Env. & Code & Description & \\ \hline
	...
	\begin{flushleft}
		\footnotesize
		C: responsibility of the cloud service provider; \\
		U: responsibility of the user.
	\end{flushleft}
\end{table}

The most popular cloud services are \mync{public cloud services} provided by Amazon, Microsoft, Google, IBM, etc., where the services are deployed remotely on the service providers' servers. Notice that public cloud services do not make up the entire cloud service market. Many enterprises also have \mync{private cloud services} where the services are deployed on premises and the employees can connect to the cloud from the internal \myabb{Local Area Network}{LAN}. There are also \mync{hybrid cloud services} that integrate private and public clouds, allowing a user to use services provided by the public clouds while storing sensitive data in the private clouds.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Secret management, such as tokens and keys
		\item Cloud services monitoring
	\end{itemize}
	\item Computing
	\begin{itemize}
		\item \myabb{Virtual Machine}{VM}
		\item \myabb{Random Access Memory}{RAM}
	...
	\begin{itemize}
		...
		\item \myabb{Domain Name System}{DNS}
	\end{itemize}
	\item Others
	\begin{itemize}
		\item System back up
...
\section{AWS Infrastructures}

AWS infrastructures are deployed in terms of the following.
\begin{itemize}
	\item \mync{AWS Region}
	
	An AWS region refers to a physical, geographic location consisting of multiple isolated and physically separated data centers groups. Different regions may provide different cloud services at different prices. 
	
	When deploying a service, such as a \myabb{Virtual Machine}{VM}, the user often needs to be specific on which region(s) the service should be deployed. There are also global services and cross-region services.
	
	\item \mync{Availability Zone}[AZ]
	
	An AZ refers to a group of closely distributed data centers. There are at least 3 AZs in a region. It is assumed that different AZs are independent and fault-tolerant, while inside an AZ all the servers are connected via low-latency networks. 
	
	Sometimes when deploying a service in a region, multiple redundant copies will be deployed on multiple AZs by default for better data availability.
	
	\item \mync{Edge Locations}
	
	Edge locations are a global network of specialized data centers distinct from AWS Regions and AZs. They are AWS managed facilities meant to be built close to the user side to provide fast data caching and exchanging.   
...
\subsection{User, Role and Policy Management}

When a user registers an account with AWS for the first time, they get the root account. The user can log into the system with the root account, and create other users and assign roles or policies to them. All services, either created by the root account or by the regular users emerged from the root account, bill from the root account. 

Consider following the good practices when creating users.
\begin{itemize}
	\item Always enable \myabb{Multifactor Authentication}{MFA} whenever possible, especially the root account.
	\item Limit the use of root account to the minimum.
	\item Create users or user groups of different types for different tasks, and while assigning privileges follow the principle of least privilege.
\end{itemize}

AWS defines three core identity and access management concepts: user, role, and policy. They are introduced below.
\begin{itemize}
	
	\item User and user group
	
	An \mync{AWS user} is a virtual representation of a person or an application that requires access to AWS resources. Human users authenticate with a username and password to access the \mync{AWS Management Console}, while both users and applications can authenticate programmatically using an \mync{access key ID} and \mync{secret access key} through the \myabb{command line interface}{CLI} or \myabb{application programming interface}{API}.
	
	By default, a user has no permissions. It merely identifies ``who is speaking''. All privileges come from the role and/or policy attached to the user. When a user is added to an \mync{AWS user group}, they automatically inherit all permissions granted to that group. A user may belong to no group (not recommended), one group, or multiple groups.
	
	\item Role
	
	An \mync{AWS IAM role} is a collection of permissions that grant access to specific AWS resources for a particular task. Unlike a user, a role is not about ``who is speaking'' but about ``what actions are allowed''. The permissions of a role are determined entirely by the policies attached to it.
	
	Roles can be assumed by users, applications, or AWS services. When an entity assumes a role, it temporarily gains the permissions defined by that role. Roles are commonly used for temporary privilege elevation, cross-account access, or granting AWS services the permissions needed to perform specific operations. The \myabb{AWS Security Token Service}{AWS STS} manages the temporary credentials associated with role assumption.
	
	Because role credentials expire, using roles for long-term or routine permissions is discouraged. If an entity consistently requires certain privileges, attach the necessary policies directly rather than relying on continuously renewed role sessions.
	
	\item Policy
	
	An \mync{AWS IAM policy} is a JSON document that defines permissions. It specifies the principal (who can claim the permissions), the action (what operations are allowed), and the resource (which AWS resources the actions apply to). A policy can be attached to users, user groups or roles.
	
	AWS provides many pre-built policies for common use cases, and users may also create custom policy documents tailored to their specific requirements. An example of a policy is given below.
	\begin{lstlisting}
{
	"Version": "2012-10-17",
	"Statement": [
	...
	\end{lstlisting}
	which allows a user to take all actions on all resources. This policy is often attached to the administrators.
\end{itemize}
...
Notice that IAM database authentication has some limits. For example, it does not support all databases on AWS. Cloud activities done via IAM database authentication is sometimes not logged by monitoring services. It cannot be efficiently scaled up when there are many connections.

\section{AWS Storage}

There are at least the following storage types in AWS.
\begin{itemize}
	\item Object storage: the storage of non-executable files, similar with OneDrive, Google Drive, etc.
...
\end{itemize}
Notice that many AWS managed services also come with storage, for example, AWS managed database. Those storage is not covered in this section, as they are entirely managed by AWS and are never exposed to the user.
...
https://<bucket name>.s3.<region>.amazonaws.com/<file name>
	...
	\item Different storage tiers are available, each with different features and cost. More to be introduced later.
	\item Server-side data encryption can be enabled.
\end{itemize}
...
\begin{itemize}
	\item Key: the name of the object or file, including prefix (parent directories).
...
It is worth mentioning that versioning is disabled by default, and the user can activate versioning for each object. Once enabled, version ID is activated and it cannot be terminated, but only be suspended.
...
As briefly mentioned earlier, S3 provides different storage tiers. Versioning can be integrated with life cycle rules, with which older versions of a file can me automatically moved into a low-cost storage tier. More about storage tiers are introduced in Section \ref{apsec:aws-s3-storage-tier}.

\subsection{AWS S3 Storage Tiers} \label{apsec:aws-s3-storage-tier}
...
\begin{table}[!htb]
	...
	\begin{tabularx}{\textwidth}{lcc}
		\hline
		\multirow{2}{*}{Tier} & Storage$^*$ & IO$^{**}$   \\ 
		& (/GB month) & (/$1$K request) \\ \hline
	...
	\begin{flushleft}
		\footnotesize
		$^*$ The average cost per TB may also be affected by the total storage size. The larger the total size, the cheaper per TB may cost. \\
		$^{**}$ Write and read cost may differ. The table lists only the cost for read requests.
	\end{flushleft}
\end{table}

From Table \ref{tab:s3tiers}, it can be seen that different storage tiers are designed for different purposes. For example, while S3 Standard is often used for general purpose storage, S3 Glacier can be used for data back up. Additional limits may apply. For example, S3 Glacier often requires a minimum storage duration of $90$ (Instant Retrieval) or $180$ (Deep Archive) days. Objects deleted prior to the minimum storage duration incur a pro-rated charge.

\subsection{AWS S3 Storage Protection}

The following protections can be enabled for AWS S3.
\begin{itemize}
	\item Data lock
	
	Data lock can be used to prevent data from being accidentally deleted or modified. AWS S3 uses S3 Object Lock that stores objects using write-once-read-many model. When S3 Object Lock is enabled, the data, once written, cannot be modified or removed. 
	
	S3 Object Lock can be applied on single object or on the entire bucket. S3 Object Lock can work in governance mode where only specified persons can change the object, or in compliance mode where nobody (not even the root user) can change the object within a certain period of time.
	
	\item Data encryption
	
	Data encryption can be used to prevent data leakage.
	
	It is possible to enforce the use of HTTPS and SSL/TLS when transmitting data. Server-side encryption can be enabled or enforced, and there are several modes for that, including SSE-S3 (S3 built-in encryption), SSE-KMS (AWS key management based encryption) and SSE-C (customer key management based encryption).
	
	\item Data access control
	
	Data access control can be used to manage the services and networks that can access the stored items.
	
	S3 Access Point can be used to manage virtual networks that can access an S3 bucket. \myabb{Multifactor authentication}{MFA} can be used on S3 when performing deletion.
	
	\item Monitoring services
	
	Monitoring services can be used to monitor suspicious behaviors and notify the users of such behavior if any.
	
	AWS has system-wise services to monitor the system behavior and notification services to notify the user on different situations.
\end{itemize}

\subsection{AWS S3 Performance Optimization}

In this context, ``performance'' refers to the maximum IO in a given period of time. There are multiple ways to improve S3 performance.
\begin{itemize}
	\item Read
	\begin{itemize}
	...
	\item Write 
	\begin{itemize}
		\item Use multipart uploads for fast file uploading. Multipart upload is compulsory for files with size larger than $5$GB.
	\end{itemize}
\end{itemize}
...
\mync{AWS Elastic Block Store}[AWS EBS] is the default block storage of AWS. AWS EBS is often attached to a \myabb{virtual machine}{VM} and used as its disk storage. 

There are two types of EBS volumes, namely the \myabb{solid-state drive}{SSD} and the \myabb{hard disk drive}{HDD}. SSD is faster than HDD and it is recommended for general purposes. HDD can be used only if the VM requires large data storage space with less cost. For a VM, the OS and boot script must be saved in SSD.
...
\mync{AWS EBS Snapshot} refers to the point-in-time snapshot of EBS volumes. By default, EBS Snapshot data is encrypted. EBS Snapshot is useful as a VM instance back up. It can also be used to mirror the deployment of an existing VM instance. EBS Snapshot can be shared among different AWS accounts. 

\myabb{Amazon Machine Image}{AMI} is the AWS maintained re-usable image provided by AWS that contains information required to launch an VM instance. There are two types of AMI, and EBS Snapshot is one of them. 

\begin{mdframed}
\noindent \textbf{Two AMI Types}

AMI has two types, the EBS Snapshot and the AWS EC2 Instance Store. The former is a snapshot of the hard drive of a VM. The later is a launching template of a VM. Both of them can be used to launch VM instances, and both of them can be stored and shared. That said, there are important differences. A brief comparison is given below.

Consider two VM instances, one launched from an EBS snapshot, and the other launched from an Instance Store. Both VM instances can be rebooted safely without losing any new data added to the VM after launching. When the VM instances are shutdown, the former one has the choice of writing the new data back to the EBS Snapshot. In this way, the EBS Snapshot becomes incremental. The user can continue left work at a later time by launching a new VM instance using the new EBS Snapshot. 

However, the same cannot be applied to Instance Store. An Instance Store cannot be generated from a VM instance. In other words,  Instance Store is stationary and not incremental by nature. When the second VM instance is stopped, should the data in the VM needs to be preserved, the data must be exported using other methods and cannot be written back to an Instance Store.

\end{mdframed}

\subsection{AWS Elastic File System}

\mync{AWS Elastic File System}[AWS EFS] refers to the shared storage that can be mounted by multiple VMs. The VMs can use EFS to share information. EFS provides read-after-write consistency. There are the following different types of EFS.
\begin{itemize}
...
Security Group is stateful. Although inbound and outbound allowed requests are defined separately, they may ``overwrite'' each other in practice. This is different from Network \myabb{Access Control List}{ACL} that will be introduced in later Section \ref{ap:aws_network_acl} which is the stateless firewall for a subnet where all inbound and outbound connectivity must be explicitly defined.
...
Notice that the use of Security Group is not limited to EC2, but also other AWS resources such as \myabb{AWS Relational Database Service}{AWS RDS}, AWS Redshift, Elastic Load Balancing, Lambda, etc. Most of these services are closely related to EC2 and computing.
...
When an EC2 is launched, the user can log into the EC2 instance via several ways. One of the easiest and most commonly used way is introduced below.
\begin{enumerate}
  \item When launching an EC2 instance, the user should create (or use an existing) key pair and attach it to the EC2 instance. This involves downloading the private key to the user's local machine. Later, the user can remote log into the EC2 instance with the private key. Consider changing the permission of the private key using \texttt{chmod 400 <key name>.pem}.  When launching the EC2 instance, select allow SSH login. This will enable SSH login in the Security Group of the EC2 instance.
  \item Once the EC2 instance is launched, get its public IP address from AWS console.
  \item SSH log into the EC2 instance using the IP address and the private key as follows:
  \begin{lstlisting}
...
\begin{enumerate}
    \item Remote log into the system as introduced earlier. Note that at this point it is simply an individual VM and has no access to any AWS service. The EC2 instance does not identify the user. The private key \verb|.pem| file cannot be used to identify the user who log into the system.
...
\myabb{AWS Virtual Private Cloud}{AWS VPC} is the AWS managed networking services to build virtual \myabb{Local Area Network}{LAN} and gateways. Other services relevant to networking, such as \myabb{Domain Name Service}{DNS} tool Route 53, are also introduced.

VPC is a very important component in AWS architecture. It groups services and virtual resources into subnets and builds connectivity within and across these subnets. Networking with AWS VPC is a complex topic. Many new concepts such as subnet, gateway, route table, endpoint, etc. are introduced in this chapter. That said, the chapter only introduces the basics of AWS VPC.

Before the introduction of AWS VPC, the following preliminary concepts are briefly reviewed.
\begin{itemize}
	\item \mync{Classless Inter-Domain Routing}[CIDR]: a commonly used IP address representation of a subnet. 
	
	CIDR is given in the form of \verb|x.x.x.x/y|, where \verb|x| is a value between $0$ and $255$, i.e., an $8$-digit binary value. This makes \verb|x.x.x.x| a $32$-digit IP address. The value of $y$ indicates the fixed number of digits among the $32$ digits shared within the subnet.
	
	For example, consider \texttt{192.168.1.0/24}. Among the $32$ digits, the first $24$ digits are fixed, and the last $8$ digits are flexible. This makes the IP address allocation \texttt{192.168.1.0} to \texttt{192.168.1.255}, with $256$ total IPs for the subnet. The value of $y$ decides the number of IP addresses in the subnet. The larger $y$, the more fixed digits in the subnet, hence the less IP addresses. For example, $16$, $65536$ addresses, $24$, $256$ addresses, $26$ represents $64$ addresses, and $32$ a single address.
	
	CIDR is used almost everywhere in AWS to represent IP addresses of a subnet. It is widely used when setting up security groups, route tables, etc.
	...
	\begin{itemize}
		\item Large network: \texttt{10.0.0.0/8}, widely used by companies.
		\item Medium network: \texttt{172.16.0.0/12}, widely used by small institutes or office networks.
		\item Small network: \texttt{192.168.0.0/16}, widely used by home routers
	\end{itemize}
	When a router sees the above IPs, it will take them as private IP addresses. Most IP address other than the above will be taken as public IP address, with some exceptions, for example \texttt{127.0.0.1} for loopback, etc.
	...
	Note that NAT will be unnecessary in the future with the spread of IPv6, which will be able to assign globally unique IP address for each and every smart device.
\end{itemize}

\begin{mdframed}
\noindent \textbf{IPv6 and Its Advantages over IPv4}

IPv6 is the next generation IP address that is more powerful than IPv4. IPv6 differs from IPv4 in many aspects, the most important of which is to have a far larger number of available addresses. IPv6 can produce over $3.4\times 10^{38}$ unique addresses. All smart devices can have their globally unique public addresses, and NAT is likely to be unnecessary. 

Since there are so many IPv6 addresses, it is improbable to scan a range of IPv6 address. With IPv4 addresses, a hacker can scan the entire address space in minutes and if he decides to launch an attack to all these IP addresses, all devices with public IP addresses will be affected. But with IPv6 address, they can hardly scan even a small portion of it. It will be more challenging for a hacker to attack a machine unless they know the exact IPv6 address of the machine in advance. It is almost impossible to scan-and-find a machine, as the distribution of valid IP addresses will be sparse.

As of this writing, IPv4 is still the most widely adopted IP address form.

\end{mdframed}

AWS VPC has very good support of IPv6. Almost everything introduced in the remainder of this section can be set up with both IPv4 and IPv6 addresses.
...
AWS VPC defines a stationary list of CIDR blocks of private IP addresses. Each CIDR defines a subnet. In AWS VPC, there is a size limit to each CIDR that ranges from \texttt{/28} ($16$ addresses) to \texttt{/16} ($65536$ addresses). That means, the size of a subnet in an AWS VPC ranges from $16$ to $65536$. In each subnet, the first $4$ IP addresses are reserved by AWS.

In the remainder of this section, the tools that AWS VPC uses to manage IP address assignments, connectivity and access controls are introduced, mainly including
\begin{itemize}
  \item Gateway: router
...
\subsection{AWS VPC Access Control List} \label{ap:aws_network_acl}

Network \myabb{Access Control List}{ACL} is the firewall defined at the subnet level, and it applies to all instances within that subnet. It is essentially a collection ranked inbound and outbound rules that allowing or denying traffics with user specified IP addresses, port and protocol, represented by tables. The default inbound and outbound rules are given in Tables \ref{tab:aws_networkacl_inbound} and \ref{tab:aws_networkacl_outbound}. Notice that though the two tables look similar, they are two separate tables.

\begin{table}[!htb]
	\centering
	\caption{AWS Network ACL default inbound rules.} \label{tab:aws_networkacl_inbound}
	\begin{tabular}{cccccc}
		\hline
        Rule \verb|#| & Type & Protocol & Port range & Source & Allow / Deny \\ \hline
        $100$ & All IPv4 traffic & All & All & \texttt{0.0.0.0/0} & Allow \\
        $101$ & All IPv6 traffic & All & All & \texttt{::/0} & Allow \\
        $*$ & All traffic & All & All & \texttt{0.0.0.0/0} & Deny \\
        $*$ & All IPv6 traffic & All & All & \texttt{::/0} & Deny \\
		\hline
	\end{tabular}
	\begin{flushleft}
		\footnotesize
		A rule with a smaller rule number has a higher priority over a rule with larger rule number.
	\end{flushleft}
\end{table}

\begin{table}[!htb]
	\centering
	\caption{AWS Network ACL default outbound rules.} \label{tab:aws_networkacl_outbound}
	\begin{tabular}{cccccc}
		\hline
        Rule \verb|#| & Type & Protocol & Port range & Source & Allow / Deny \\ \hline
        $100$ & All IPv4 traffic & All & All & \texttt{0.0.0.0/0} & Allow \\
...
Notice that Network ACL is stateless. For bidirectional communication, both inbound and outbound connectivity need to be allowed. This is different from Security Group (the firewall of a device such as EC2 instance; recall Section \ref{ap:aws_ec2_security_group}), which is often stateful.
...
\subsection{AWS VPC Peering}

VPC Peering allows seamless connectivity of devices in two VPCs, just as if they were in the same VPC. The two VPCs peered together cannot have overlapped CIDRs. VPC Peering is not transitive. 
