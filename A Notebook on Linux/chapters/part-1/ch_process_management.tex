...
Managing processes is one of the essential tasks of an OS. In a Linux system, the user can manage processes in the prompt console using bash commands. Details are introduced in this chapter.

\section{General Introduction to Process and Thread}

Process and thread are two fundamental units that the OS can manage to influence the behavior of a running program. Details are introduced as follows.

\subsection{Process}

\mync{Process}, also known as \mync{task}, represents an instance of a program or an application that is running on the machine. The OS manages the applications and their corresponding resources by managing the processes.

To improve the efficiency of the system, especially making better use of the \myabb{central processing unit}{CPU}, the OS allows multiple processes to share the computational capability and memory of the system. The intention is to prevent a program from ``locking'' the CPU while it is waiting for I/O (such as reading hard drive, communicating over the internet, waiting for human input from keyboard, printing to screen or printer), which would have otherwise waste the CPU capability. This is demonstrated by Fig. \ref{ch:pm:fig:processflow}.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=\textwidth]{chapters/part-1/figures/processflow.png}
...
Fig.~\ref{ch:pm:fig:processstatetransfer} shows the possible states that a process might be at. Note the differences between ``blocked'' and ``suspended''. ``Block'' indicates that the process is waiting for others such as additional inputs or another process, while ``suspended'' indicates that the process is put on hold for some reasons. When a process is offloaded from the CPU, its context is moved from the CPU registers to the PCB of the process.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=\textwidth]{chapters/part-1/figures/processstatetransfer.png}
...
A process usually has a relatively isolated environment, and it does not share memory storage with other processes. Special communication mechanisms, which are often referred to as ``pipes'', are required for processes to talk to each other. Cross-process communication requires OS level controls.

\subsection{Thread}

A \mync{thread} is like a work dispatch inside a process. There can be multiple threads in a process, as shown in Fig \ref{ch:pm:fig:threadinprocess}. Each thread has its own CPU register values and stack, and hence is able to run on a CPU core separately. All threads in a process naturally share the same codes, memory and work space.
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.6\textwidth]{chapters/part-1/figures/threadinprocess.png}
...
\begin{itemize}
  ...
  \item Sharing memories and resources among threads in a process is easier than sharing across processes because they naturally share address space.
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.8\textwidth]{chapters/part-1/figures/pscommand.png}
	\caption{Execution of \texttt{ps -ef} command.} \label{ch:pm:fig:pscommand}
\end{figure}

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.8\textwidth]{chapters/part-1/figures/topcommand.png}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{chapters/part-1/figures/priority.png}
