...
{Linux \myabb{Command Line Interface}{CLI}, usually known as the ``shell'' or ``terminal'', is the most available, flexible and powerful tool for the users and programs to interact with the OS and perform actions.
...
Some other shells such as ``C Shell'' and ``Korn Shell'' are also popular among certain users or certain Linux distributions. For example, C Shell supports C-like shell programming, which is sometimes more convenient than bash. In case where the Linux distribution does not have a certain type of shell the user needs, the user can install it just like installing any other software.

\subsection{Prompt}

With a newly started shell, a string (usually containing username, hostname, current working directory, etc.) followed by either ``\verb|$|'' or ``\verb|#|'' should appear. Following the string, the user can key in shell commands. The string may look different on different machines. An example is given below.
\begin{lstlisting}
<username>@<hostname>:~$
\end{lstlisting}
...
\subsection{Redirection of Commands Outputs}
...
\section{Basic Shell Script Programming Syntax}
...
\subsection{Typical Shell Script Practice}
