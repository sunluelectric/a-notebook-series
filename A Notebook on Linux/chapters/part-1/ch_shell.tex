...
Linux's default \mync{Command Line Interface}[CLI], known as the \mync{shell} or \mync{terminal}, is the most available, flexible and powerful tool for the users and programs to interact with the OS and perform actions. Technically speaking, shell is not compulsory for casual users when graphical desktop environment is available. Still, it is strongly recommended that the users shall understand at least the basics of shell since it is more flexible and powerful than the graphical desktop and hence can become handy from time to time when configuring and troubleshooting the system and software applications. On those machines where no graphical desktops are installed, shell is critical.
...
Command \verb|alias| is used to create short-cut keys for commands and associated options, which makes it more convenient for the system operators to work on shell. Some alias has already been created automatically when shell is started. Use \verb|alias| to check the existing alias in the shell. An example is given below.
