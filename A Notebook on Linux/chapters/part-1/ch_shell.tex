...
This chapter introduces Linux shell, Linux's default command line interface. Basic commands are covered.

Linux shell will be used frequently in the remainder of this notebook.

\section{Shell Basics}

Linux's default \myabb{command line interface}{CLI}, known as the \mync{shell} or \mync{terminal}, is the most available, flexible and powerful tool for the users and programs to interact with the OS and perform actions. Technically speaking, shell is not compulsory for casual users when graphical desktop environment is available. Still, it is strongly recommended that the users shall understand basics about shell since it is more flexible and powerful than the graphical desktop and hence can become handy from time to time when configuring and troubleshooting the system and software applications. On machines where no graphical desktops are installed, the use of shell is critical.
...
An example of a shell script that calculates Fibonacci series is given below. Notice that in practice shell is mainly used for interaction with the OS and not for data processing or numerical calculation. The example is only for demonstration.
...
For the term \mync{root user} or root for short, we are referring to a special user with username and \myabb{User ID}{UID} ``root'' and 0 respectively. This UID gives the user the administration privilege over the machine, such as adding/removing users, changing ownership of files, etc. For security reasons, root user shall not be used on a daily basis. The root user's authentication is often deactivated by default. This can be done by setting its login password to invalid.

Notice that the root user is different from a \mync{sudoer}, the latter of which is basically a regular user equipped with \mync{sudo privilege}. Different sudoer may have different sudo privilege setups. A sudoer may be able to temporarily switch to the root user using \verb|sudo su| if their sudo privilege setup is so configured. A demonstration is given below.
\begin{lstlisting}
...
\end{lstlisting}
where \verb|<command>| is either built-in or from an installed application, often as part of its CLI. Commonly seen commands are introduced in the remainder of this section. Commonly used applications and their associated shell commands will be introduced in later chapters.
...
The following command \verb|lshw| lists down hardware information in detail. Notice that sudo privilege is recommended when using this command to give more detailed and accurate information of the system. Sometimes displayed information can be too detailed. Use \verb|-short| argument with the command to shorten the output.
\begin{lstlisting}
$ sudo lshw
<hostname>
...
$ alias pwd='pwd; ls -CF'
\end{lstlisting}
To permanently add alias to the shell, the alias needs to be added to \verb|~/.bashrc|, which is a piece of shell script that is automatically executed each time a new shell is started by the user. It is sometimes known as the \mync{user script}. User script can be used to customize bash shell environments. More about user script can be found in Section \ref{ch:sb:subsec:customizeshell}.
...
\item \verb|/home/<username>/bin|: Commands introduced by a particular user.
\end{itemize}
...
<command location>
\end{lstlisting}
If an external command is not registered under \verb|PATH|, the command needs to be referred by not only its name but also its path in the system, so that shell can locate it. Notice that shell built-in commands (not Linux built-in commands, which are often considered as external commands to shell) such as \verb|alias| and \verb|pwd| can be executed anyway even if they are not covered by \verb|PATH|.
...
\begin{table}[!htb]
	...
	\begin{tabularx}{\textwidth}{lX}
		...
		\verb|~/.bashrc| & Bash configuration for current user, also known as user script in some contexts. It executes upon the user starts a shell. \\ 
