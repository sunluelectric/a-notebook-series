...
Linux, just like any other OS, monitors and maintains the software installed on the system. Different Linux distributions may use different tools to manage software. Commonly seen software management tools are introduced in this chapter. 

Some software, such as databases, runs as services. Linux also manages services. Service management is not covered in this chapter, but will be introduced in Chapter \ref{ch:services}.

\section{Software Management Tasks}

When it comes to software management, an OS should be able to perform at least the following tasks.
\begin{itemize}
	\item Installation
		\begin{itemize}
			\item Install software using local installation package.
			\item Find and install software from online repositories.
			\item Find and install software dependencies from online repositories.
		\end{itemize}
	\item Querying
		\begin{itemize}
			\item Query installed software.
			\item Query install-able software and dependencies from online repositories.
		\end{itemize}
	\item Updating
		\begin{itemize}
			\item Trace the version of software.
			\item Trace available versions of software from online repositories.
			\item Update software to specified version.
		\end{itemize}
	\item Removal: uninstall software and dependencies.
\end{itemize}
...
Different Linux distributions may use different package formats and package management tools. The most well-known ones include:
\begin{itemize}
	\item \myabb{Red Hat Package Manager}{RPM}
...
RPM (\verb|.rpm|) is the preferred package format for Red-Hat-based distributions such as RHEL, Fedora, CentOS and Oracle Linux. 

In early days \verb|rpm| command was directly used to manage RPM packages. Nowadays, tools such as \verb|yum| and \verb|dnf| have been developed for better user experience and more efficient package management. 

\subsection{Brief Introduction}

RPM contains rich information, including not only the payload of the software such as commands, configuration files, libraries and documentation, but also metadata such as the source of the package and the dependencies.

An RPM package should be named following the convention, where the software name, the version, the build, and other metadata should be reflected. An example is given below. Use \verb|rpm -q| followed by the software name \verb|python3| to query the package as follows. If the package is installed, its full name should be returned.
\begin{lstlisting}
$ rpm -q python3
...
Software can be installed with an RPM package. An RPM package comes from the upstream software provider, who collects source codes, binary files, and other data, builds the software, and signs the package.
...
DNF is a further improvement of YUM with better dependency resolution and less memory usage. In many occasions, command \verb|yum| can be replaced by \verb|dnf| seamlessly. In some Linux distributions such as RHEL 9, \verb|yum| is used as an alias for \verb|dnf|. In the remainder of the section, only DNF is discussed.

The basic syntax of DNF is
\begin{lstlisting}
$ dnf <operation> <package>
\end{lstlisting}
where allowed \verb|<operation>| are summarized in Table \ref{tab:dnf-operations}.

\begin{table}[!htb]
	\centering \caption{Operations for DNF package manager.}\label{tab:dnf-operations}
	\begin{tabularx}{\textwidth}{lX}
		\hline
		Operation & Description \\ \hline
		\verb|info| & Provide the basic information of a package. \\
		\verb|search| & Search the online repositories for a package. \\
		\verb|install| & Install packages. \\ 
		\verb|check-update| & Check updates of a package from online repositories without downloading or installing them. \\
		\verb|update| & Update packages safely. \\
		\verb|upgrade| & Update packages more aggressively than \verb|update|. \\ 
		\verb|downgrade| & Revert to a previous version of a package. \\
		\verb|reinstall| & Reinstall packages. \\
		\verb|remove| & Remove packages. \\
		\verb|autoremove| & Remove dependencies not relevant to currently installed applications. \\
		\verb|exclude| & Exclude a package from installation or update, either temporarily or permanently. \\
		\verb|list --installed| & List installed packages. \\
		\verb|list --available| & List available packages. \\
		\verb|list --upgrades| & List available upgrades for packages. \\
		\verb|list --autoremove| & List dependencies not bounded to any installed application. \\
		\verb|deplist| & Query dependencies of a package. \\
		\verb|-q| & Query repository for a package. \\
		\verb|history| & Check historical transactions. \\
		\verb|clean all| & Clean cache. \\
		\hline
	\end{tabularx}
\end{table}
...
Package \verb|kernel*| such as \verb|kernel-core| are the main package of Linux kernel. Package manager \verb|dnf| can be used to query and update these packages. It is possible to install multiple kernels with different versions on the same physical machine, in which case there will be a boot entry for each kernel and the user can select which kernel to boot upon restarting the system.
...
Kernel modules are scripts used to extend OS functionality. The user can develop and deploy their own modules to customize the OS. To list existing modules and to query information of these modules, with \verb|kmod| installed, use
\begin{lstlisting}
$ lsmod
$ modinfo <module name>
\end{lstlisting}
...
$ sysctl -a
\end{lstlisting}
lists all kernel parameters values.
