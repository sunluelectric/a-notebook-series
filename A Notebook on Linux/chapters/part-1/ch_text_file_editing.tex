...
\section{General Introduction to Vim} \label{sec:vimintro}

\mync{Vi IMproved}[Vim] is a free and open-source software developed by Bram Moolenaar et al. It is an expansion to the Vi text editor to include features such as syntax highlighting, and has become the default text editor of many Unix/Linux based operating systems.

Some people claim Vim, together with its variations, to be the most powerful text file editor and \myabb{integrated development environment}{IDE} for text editing on a Linux machine (and potentially on all computers and servers). The main reasons are as follows.
\begin{itemize}
...
Vim defines different modes for different type of tasks. \mync{Normal mode}, \mync{insert mode}, \mync{visual mode} and \mync{command line mode} are the four modes defined in Vim, with their functions summarized in Table \ref{ch:tfe:tab:vimmodes}.
...
\begin{table}[!htb]
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    Insert & It is used to insert keyboard inputs into the text just like commonly used text editors. \\ 
...
This section introduces basic and commonly used Vim operations. The mode switch between normal mode and insert mode are introduced which would allow the user to perform basic file editing. Commonly used normal mode operations such as cut, copy and highlight search are introduced. Visual mode is briefly introduced.

\subsection{Basic File Editing}
...
$ vim <filename>
\end{lstlisting}
to create a new file \verb|<filename>| assuming it does not already exist, and open the file using Vim. Notice that in some Linux versions, command \verb|vi| might be aliased to \verb|vim| by default.

In the opened file, use \verb|i|/\verb|a| (insert/append) to switch from default mode to insert mode, and \verb|Esc| to switch back to normal mode. Notice that in normal mode, the cursor is on a character. When using the insert command, the insertion cursor is put to the left of that character, whereas when using the append command, the insertion cursor is put to the right of that character. In normal mode, use \verb|h|, \verb|j|, \verb|k|, \verb|l| to navigate the position of the cursor. If a modern keyboard is used, the left, right, up and down arrows can also be used to navigate the position of the cursor in both normal and insert modes. 

To quickly navigate, consider using \verb|b| and \verb|w| rather than \verb|h| and \verb|l| to move cursor by words instead of characters. Use \verb|0| (zero) to jump to the beginning of the line, or \verb|$| to the end of the line. Use \verb|gg| to jump to the first line of the file, and \verb|G| to the last line. Use \verb|:<number>| to jump to a specific line of the file.

Finally, in normal mode, use \verb|:w| to save the file, and \verb|:q| to quit Vim, or use \verb|:wq| to save and quit Vim.
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.8\textwidth]{chapters/part-1/figures/vimbasicmodeswitching.png}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.8\textwidth]{chapters/part-1/figures/vimbasicoperationflowchart.png}
...
For example, consider command \verb|u| for undo. It is not an operator–motion command, but a standalone command that does not take a motion argument.

To repeat an operation multiple times, prefix the command with a number indicating how many times it should be executed, and the syntax becomes
\begin{lstlisting}
<number><operator><option>
\end{lstlisting}

Delete, cut, copy and paste are mostly done in normal mode, and they serve as good examples for the above rule.
...
In Vim, operator \verb|d| deletes text. It can be used to perform cut-and-paste as well, since the deleted content using \verb|d| is automatically stored in the clipboard. Operator \verb|d| requires a motion to define the range. For example, \verb|dl| deletes one character to the right (the character at the cursor), while \verb|dh| deletes one character to the left. Motion can be prefixed. For example, \verb|d20l| deletes 20 characters to the right, where ``20l'' serves as the motion.

The operator \verb|d| can be used with other motions. For example, \verb|w| is a word motion and its range is to the first character of the next word. Consider sentence 
\begin{lstlisting}
William Shakespeare (c. 23 April 1564[b] - 23 April 1616) was an English playwright, poet and actor.
\end{lstlisting}
with the position of the cursor at ``S'' in ``Shakespeare''. Operation \verb|dw| deletes ``Shakespeare '' (notice that the space after Shakespeare is also deleted). When the cursor is in the middle of a word, \verb|dw| deletes from the cursor to the beginning of the next word. For instance, if the cursor is on the ``k'' in ``Shakespeare'', \verb|dw| deletes ``kespeare ''. Motion \verb|b| works similarly, except that it traces words backward to the left. To delete the whole word regardless of cursor position, use the ``inner word'' motion \verb|iw|. For example, \verb|diw| deletes the entire word under the cursor but keeps the following space.
...
The operator \verb|c| works similarly with \verb|d|, except that it automatically switches to insert mode after removing the selected text. 

To copy text into the clipboard, use \verb|y| (short for ``yank''), followed by a motion to indicate the range of text. The motions follow Table \ref{ch:tfe:tab:motion}. To paste the most recently yanked or deleted text after the cursor, use \verb|p|. No motion is required.
...
In normal mode, use \verb|/<keyword>| followed by \verb|Enter| to search the file for a keyword. Keys \verb|n| and \verb|N| are used to navigate through the search results. The command \verb|zz| re-centers the current cursor line to the middle of the screen. 
...
In command-line mode, the prefix \verb|!| allows interaction with the shell. For example, consider a case where the file can only be edited by a sudoer. A user, who has sudo privilege, opened the file in regular mode and forgot to use sudo. After making changes to the file, they cannot save it using \verb|:w| due to permission failure. In this case, consider using
\begin{lstlisting}
:w !sudo tee %
\end{lstlisting}
to save the file. Here, \verb|tee| is a Linux command that takes standard input and writes it to a file, and \verb|%| refers to the current file.
...
With the basic operations introduced so far, we are able to create and edit text files of any kind. We can now customize the user profile for better user experience. Notice that the customization is completely optional and personal. This section only introduces the basic setups such as re-mapping keys and creating user-defined shortcuts. Everything introduced here are merely examples and it is completely up to the user how to design and implement his own profile.
...
Advanced operations such as macros and multiple line selection using visual mode are introduced in this section.

\subsection{Vim Macros}
...
To repeat the recorded procedures, use \verb|@<macro-name>|. In this example, just key in \verb|@s| in normal mode, and the file should appear as
\begin{lstlisting}
image: 'apple.jpg'
ttl: 5
image: 'pear.jpg'
...
In any of the above visual mode, use \verb|:normal + <operation>| to execute normal mode operation(s) for each line. This allows convenient editing of multiple lines of text all together. For example, use \verb|V| to select a few lines of contents, followed by \verb|:normal 0iprefix-|. This will insert \verb|prefix-| to all the lines, as if \verb|0iprefix-| is executed in normal mode to all the lines separately, where \verb|0| (zero) is a short key to navigate to the beginning of the line, \verb|i| switches to insert mode, and \verb|prefix-| is entered as the content.

Alternatively, in visual-block mode after selecting a block of content, use \verb|I| to enter insert mode and insert content in the first row of the selected block. Then exit the insert mode. The changes made to the first row, such as adding the prefix, will be apply to all selected rows.
...
\mync{NeoVim} is a fork of Vim. It is a community-driven project with diversified contributors. NeoVim is often regarded as a more cutting-edge version of Vim in the sense that new features usually come sooner in NeoVim than in Vim. NeoVim also has a better and more native support for Lua language. The configuration file for NeoVim, i.e., the counterpart of \verb|vimrc|, can be built by Lua files. 

Vim is intended to be a single-thread application which is fine as a text editor. However, it limits its ability of calling and running other shell services simultaneously. NeoVim, on the other hand, uses multi-thread framework, hence does not pose this issue. Later on Vim added the support to allow plug-ins to trigger multi-thread processes.
...
Apart from Vim and NeoVim, many other text editors are also widely used in Linux, each with different features. For demonstration purpose, Vim and other text editors are used to open a shell script that calculates the first 10 elements of Fibonacci series.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.6\textwidth]{chapters/part-1/figures/vim_fib.png}
	\caption{Vim (with customization).}
\end{figure}

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.6\textwidth]{chapters/part-1/figures/nano_fib.png}
	\caption{The nano editor.}
\end{figure}

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.6\textwidth]{chapters/part-1/figures/emacs_fib.png}
	\caption{The emacs editor.}
\end{figure}

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.6\textwidth]{chapters/part-1/figures/gedit_fib.png}
	\caption{The gedit editor.}
\end{figure}

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.6\textwidth]{chapters/part-1/figures/vscode_fib.png}
