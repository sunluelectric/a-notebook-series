...
\end{lstlisting}
where the first \verb|/| represents the root directory, and sequential \verb|<directory>/| represents a subdirectory under its parent.

Upon Linux installation, a \mync{filesystem hierarchy} is created. A user can create new files or directories under this hierarchy framework, but should not change the framework itself. The hierarchy is given in Fig. \ref{ch:fm:fig:hierarchy}. Notice that different Linux distributions may differ slightly on how the architecture looks like. The ``\verb|/|'' in the figure, as introduced, stands for the root directory, and ``\verb|root|'' in the figure is a subdirectory under \verb|/| whose directory name is ``root'' and it is used store root user related documents. They are two different directories.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.8\textwidth]{chapters/part-1/figures/linux_file_hierarchy.png}
	\caption{An example of Linux file system hierarchy.} \label{ch:fm:fig:hierarchy}
\end{figure}

A regular user's home directory is often located at \verb|/home/<user name>|. When logging in as a regular user, his home directory is stored in the HOME environmental variable and can be retrieved by \verb|$HOME|. A shortcut to \verb|$HOME| is given by the tilde $\sim$ for convenience. Hence, for example \texttt{ls}~$\sim$ lists down the files and directories under his home directory.

As can be seen from Fig. \ref{ch:fm:fig:hierarchy}, the hierarchy contains quite a few pre-determined subdirectories, each serving a different purpose. For the ease of illustration, these subdirectories are roughly categorized by functionalities and accessibility shown in Fig. \ref{ch:fm:fig:directorycate}. A brief introduction to the directories are summarized in Table \ref{ch:fm:tab:hierarchyintro}.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=\textwidth]{chapters/part-1/figures/linux_directory_cate.png}
...
\mync{Magic numbers} of a file refer to the first few bytes of a file that are unique to a particular file type, for example, PNG file is hex \verb|89 50 4e 47|. Linux compare the magic numbers of a file with an internal database to decide the file types and features. Distinguishing file types using magic numbers can be more reliable than using suffixes, though a bit less intuitive.

\section{Hard and Soft Links}

A \mync{link}, in the filesystem context, refers to the mapping of a file name and path to its physical existence on the hard drive. The user and the OS can reach and manipulate the storage using the fine names and paths.

When a file is created, its file full path is used as the link. In addition to the original file name, Linux allows the user to create hard links and soft links to refer to a file. A demonstrative plot is given in Fig. \ref{fig:hardsoftlink}. More details are introduced in the rest of the section.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.8\textwidth]{chapters/part-1/figures/hardsoftlink.png}
	\caption{Original file name, hard link and soft link.} \label{fig:hardsoftlink}
\end{figure} 

\subsection{Hard Link}

As shown in Fig. \ref{fig:hardsoftlink}, a \mync{hard link} is an alternative to the file name that directly points to the stored data. Both the original file name and the hard link share the same \mync{inode number}. The inode number is a unique identifier of a file that Linux uses to map a file name to the stored data. From this sense, a hard link is equivalent to the original file name, and the original file name can be taken as the ``default hard link''.

If a piece of hard drive storage is associated with only one hard link, removing the hard link would mean removing the information on the hard drive. Usually the OS would not directly erase the information from the hard drive. Instead, it will mark the space on the hard drive as ``free'', and later other information will occupy the space and overwrite the contents. However, if a storage is associated with multiple hard links, removing one of the hard links does not clear the data, and the data is still accessible via the remaining hard links. Only when all the hard links are removed, should the OS mark the storage as free. This is because the OS traces the link account of an inode number (the number of hard links to an inode number, the original file name included), and will consider the data removable only if it drops to 0.

Hard links can be used as a back up of the original file just in case its original file name is deleted by accident. Notice that a hard link does not create a physical duplication of the data referred by the original file name on the hard drive. The hard link can only project the file from accidental deletion, but not from edition of its contents.

Use
\begin{lstlisting}
$ ln <existing link> <new hard link>
\end{lstlisting}
to create a hard link for a file.

\subsection{Soft Link}

As shown in Fig. \ref{fig:hardsoftlink}, a \mync{soft link} (also known as \mync{symbolic link}) is a second-layer point of the data. The soft link does not point to the data in the physical storage directly, bu to another hard or soft link and forms a chain. If any links in the chain is broken, the soft link becomes a dangling link and loses the reference to the data. Notice that there is a limit to the length of the chain which effectively avoids looped chain. A larger overhead of processing time is expected as the chain becomes longer.

Soft links are useful to create shortcuts or cross-file system links.

Use
\begin{lstlisting}
$ ln -s <existing link> <new soft link>
\end{lstlisting}
to create a soft link for a file.

\section{Filesystem Management} \label{ch:fm:sec:filemanagement}
...
\begin{table}[!htb]
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    \verb|touch| & Create an empty file, or update the ``last-modified'' timestamp of a file. \\ 
...
\begin{mdframed}
\noindent \textbf{Use of Wildcard Characters in Filesystem Management}
...
\begin{table}[!htb]
	...
	\begin{tabularx}{\textwidth}{lX}
		...
		\verb|?| & Matches any one character. \\ 
		\verb|[...]| & Matches characters given in the square bracket, which can include a hyphen-separated range of characters. \\
		\hline
	\end{tabularx}
\end{table}

\subsection{Query of Locations of Commands and Files}

The most commonly seen 3 searching use cases are as follows.
\begin{itemize}
...
For security and privacy reasons, \verb|locate| only shows files that the invoking user has permission to access. For example, a regular user cannot use \verb|locate| to see files in \verb|/root| or another user’s home directory.
...
\begin{table}[!htb]
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    \verb|x| & Run a file as a program. Set the directory to be the current directory; search through the directory; access metadata (file size, etc.) of files in the directory. \\
    \hline
  \end{tabularx}
\end{table}
...
\subsection{Ownership Management} \label{sec:chown}
...
\subsection{Permission Management} \label{sec:chmod}
...
\subsection{Default Permission Management}

The default ACL for a newly created file or directory is defined by \verb|umask|. Check its value using
\begin{lstlisting}
$ umask
\end{lstlisting}
Its value can be changed temporarily using
\begin{lstlisting}
$ umask <new value>
\end{lstlisting}
