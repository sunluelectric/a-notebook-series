...
This chapter discusses user account management such as creating a user and assigning roles and enabling access to resources for them.

\section{Root User Management}

Managing and protecting the root user account is a big part in Linux administration. 

For the term \mync{root user} or sometimes \mync{root} for short, we are referring to a special user with username, \myabb{User ID}{UID} and \myabb{Group ID}{GID} ``root'' and ``0'' and ``0'' respectively. The root user is created during the installation of the OS. Its home directory is by default \verb|/root| instead of \verb|/home/<user name>|. The default prompt sign for the root user is often \verb|#| instead of \verb|$|, the latter of which is used by regular users including sudoers. The root user has administrative privileges and can do almost everything without being denied or questioned by the system. 
...
\begin{table}[!htb]
  \centering \caption{The columns in \texttt{/etc/passwd}, using root user an an example.}\label{tab:etcpasswd}
  \begin{tabularx}{\textwidth}{llX}
    \hline
    Index & Name & Comments \\ \hline
    1 & Username & The username for the root user is \verb|root|. \\ 
    2 & Password & The \verb|x| signifies that the password stored in the file is encrypted. \\ 
    3 & UID & The UID of the root user is 0. \\
    4 & GID & The GID of the root user is 0. \\
    5 & GECOS & A comment field that stores additional information of the user, such as its full name, address, telephone, etc. \\
    6 & Home Directory & The home directory of the root user is \verb|/root|. \\
    7 & Login Shell & The default shell is located at \verb|/bin/bash|. \\
    \hline
  \end{tabularx}
\end{table}
...
A \mync{sudoer} refers to regular users who can temporarily elevate their privileges and execute administration commands using 
\begin{lstlisting}
$ sudo <privileged command>
\end{lstlisting}
There can be multiple sudoers in the system, each with a different set of allowed privileged commands. Depending on the configuration, a sudoer may be able to switch to the root user temporarily using \verb|sudo su| as shown below 
\begin{lstlisting}
<username>@<hostname>:~$ sudo su
[sudo] password for <username>:
root@<hostname>:/home/<username>#
\end{lstlisting}
and quit using \verb|exit|.
...
\mync{Secure Shell Protocol}[SSH] is a commonly used protocol to remote login to a Linux server. To do so, SSH remote login must be enabled on the server side as a prerequisite. 
