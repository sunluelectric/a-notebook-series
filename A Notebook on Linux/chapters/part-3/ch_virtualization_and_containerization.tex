...
The purposes of virtualization and containerization were introduced in earlier Section \ref{sec:vcintro}. Their main terminologies and technologies are briefly introduced in the remainder of this section.

\subsection{Virtualization Terminologies}
...
To solve this problem, we need a systematical and automated way of integrating and distributing computational resources to the applications in an efficient manner. Virtualization is one of the most important technologies used in this framework. Virtualization is essentially about running an application on a VM but not a real physical machine. Multiple VMs may share the same physical server. The virtual environment is emulated and managed by a special software running on the actual machine known as the \mync{virtual machine monitor}[VMM] or \mync{hypervisor}. The setup is often transparent to the applications as they perform the same way as if they were running on a dedicated physical machine.
...
\subsection{Containerization Terminologies}

Containerization is an alternative virtualization approach to VM that can also virtualize independent workspace for applications. Comparing with VM, containers are often lighter, hence more efficient for massive microservices deployment. Depending on the context, the term ``container'' may refer to container runtime, container engine or container orchestration.
...
For example, consider running a container of alpine using the command below. A screen shot is given in Fig. \ref{ch:vac:fig:dockerrunexp}.
\begin{lstlisting}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.6\textwidth]{chapters/part-3/figures/dockerrunexppart2.png}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.6\textwidth]{chapters/part-3/figures/dockerrunexppart3.png}
