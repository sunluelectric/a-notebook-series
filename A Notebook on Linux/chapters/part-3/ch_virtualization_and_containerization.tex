...
\section{Introduction} \label{sec:vcintro}

Brief introductions to virtualization and containerization are given in this section.

\subsection{Virtualization}

In a conventional data center, multiple physical servers are deployed, and each server has a single associated application. Servers may share the same \myabb{local area network}{LAN} and \myabb{network-attached storage}{NAS} for data exchanging. A big issue of this implementation is the utilization efficiency of the servers and the unevenly distributed loads, as some of the servers may not be utilized efficiently while others may be overwhelmed. To deploy a new application, a new server must be purchased, which can take months of time. With more and more servers, the asset and management cost may grow wild.

To solve this problem, we need a systematical and automated way of integrating and distributing computational resources across multiple applications on the same server. Though working on the same machine, a user or an application would usually want a private working environment not interrupted by others. In other words, they would want to virtually work on an independent machine with his own CPU, RAM, I/O, OS, drivers and storage, despite that the actual hardware is shared. This can be achieved through \mync{virtualization}, which enables running multiple operating systems on a single physical server in an uninterrupted and logically separated manner. The virtually independent computer of such kind is often called a \mync{virtual machine}[VM].

The virtual environment is emulated and managed by a special software running on the actual machine known as the \mync{virtual machine monitor}[VMM] or \mync{hypervisor}. The setup is often transparent to the applications as they perform the same way as if they were running on a dedicated physical machine.

Depending on the items to be virtualized, virtualization techniques can be categorized as follows.
\begin{itemize}
	\item Virtualization of infrastructure and equipment, mainly network resources and storage. This results in \myabb{virtual LAN}{VLAN}, \myabb{virtual private network}{VPN}, NAS and \myabb{storage area network}{SAN}.
	\item Virtualization of operating system. This results in the well-known VM and virtual desktop.
	\item Virtualization of application running environment. An example is \myabb{Java virtual machine}{JVM} that allows Java to generate consistent results while running on different machines.
\end{itemize}
There are many virtualization architectures available and the user can choose the best architecture that suits their needs.

When people talks about virtualization, it is most likely that they are talking about VM. Commonly seen terminologies in VM are listed below.
\begin{itemize}
	\item Host OS. The OS that resides on the physical machine, on which virtualization tools run.
	\item VMM. This is a software that runs on the host OS, managing all the guest OSes, providing them interface to the host OS and the hardware.
...
\begin{itemize}
	...
	\item Hardware-assisted virtualization. The hardware, such as the processor, is specifically designed to provide certain VM functions. These functions, after passing through VMM, are directed and executed on the hadrware, thus enhancing system performance.
\end{itemize}
...
\begin{mdframed}
	\noindent \textbf{Who is the boss, host OS or hypervisor?}
	
	Both the host OS and the hypervisor can run in privilege mode (also known as ``Ring 0'' in x86 architecture). However, notice that at one time there can be only one entity that runs in Ring 0.
...
Deploying a new VM generally consumes a considerably large amount of time and resources because the guest OS needs to be installed. It is hardly possible to launch a VM for each microservice when there are many of them. In this case, a more efficient approach is to deploy a single VM and place each application in an isolated environment with its own customized drivers and configurations. The technology used to deploy and manage containers is known as \mync{containerization}. The isolated environment where an application instance runs is known as the \mync{container}. 

A container is similar to a VM in the sense that it provides certain degrees of isolation from others, but it is typically ``lighter'' than a VM because it does not need to virtualize or duplicate the whole OS as VMs do. This makes containers cheaper to launch and manage, hence more suitable for massive microservices deployment. A container contains all the configuration and requirement information of an application. Running containers with the identical configuration on different platforms would consistently generate the same expected result. This has made the sharing and rapid deployment of containers remarkably easy and convenient. 

Depending on the context, the term ``container'' may refer to container runtime, container engine or container orchestration.

\mync{Container runtime} refers to the backend software that actually hosts the containers. Examples of widely used container runtimes are \verb|containerd|, \verb|runc| and \verb|cri-o|. 

It is often inconvenient for users to talk to container runtimes directly. \mync{Container engine} is the interface for a user or software to manage images and containers. Examples of container engines include docker and podman. 

Finally, \mync{container orchestration} is the software that strategically deploy, monitor, restart, and terminate the containers on servers. It can scale up and down the number of containers based on the demanding, and balance the API calls each container receives. Container orchestration is very useful in production environment of large-scale services. One of the most widely used container orchestrations is kubernetes.

Notice that though container runtime is always a must-have, container engine and container orchestration are not. A container orchestration used to talk to a container runtime through a container engine. Nowadays, many container orchestrations directly communicate with container runtimes.

\subsection{Comparison}

The similarities and differences of conventional servers, VMs, and container applications are summarized in Fig. \ref{ch:vac:fig:pcvmcontainersructure}. VMs are marked by red dotted rectangles and containers, blue dotted rectangles.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=\textwidth]{chapters/part-3/figures/pcvmcontainerstructure.png}
	\caption{System architectures of conventional servers, VMs and containers.} \label{ch:vac:fig:pcvmcontainersructure}
\end{figure}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.8\textwidth]{chapters/part-3/figures/multitaskcook.png}
...
In the case of a restaurant, recipes describes the cookers and the raw materials a dish requires. Similarly in containerization, an image contains all the information required to initialize a container. Images can be shared among machines to replicate the containers initial status even if the machines adopt different underlying infrastructures such as hardware and OS. More about container image will be introduced in Section \ref{ch:vac:sec:di}.

\section{Container} \label{ch:vac:sec:dc}

This section introduces docker and podman container engines. Notice that the scope of this notebook is limited to the use of these engines, not the technologies behind them.

When comes to user interface, docker and podman are very similar and hence can be introduced as a whole. Unless otherwise mentioned, docker and podman can be used interchangeably on everything to be introduced in the remainder of this section. In many machines, command \verb|docker| is used as an alias of \verb|podman| by default.

\subsection{Engine Installation}

Docker is a company that has developed many container-relevant technologies and products, among which is the \mync{docker container engine} (also referred as the docker engine or just docker for short), the most popular container engine available on the market as of 2026 with over $80\%$ market share. Docker container engine is free-of-charge for open-source, personal and small business projects. It is so popular that ``docker'' is sometimes used to refer to any container engine.

But does that mean docker engine is the absolutely best and perfect container engine solution?

Docker engine has surely revolutionized how containerization is used in software development and deployment, and it has been one of the most popular and beloved container engine solutions. However, it is worth mentioning that docker engine is not the only available container engine. For example, ``podman'' developed by Red Hat, is an alternative to docker. It supports the same interface as docker in its CLI and claims to have better performance and security. RHEL has already started the transition from docker to podman from RHEL 8. Nowadays, installing docker on the latest versions of RHEL is possible but tedious. On the contrary, podman often comes with RHEL installation by default. Kubernetes, a famous container orchestration, is also dropping docker support. According to its official statement, docker support will eventually be removed in a future release of Kubernetes. Some may even argue that ``containers are alive, but the role that docker plays is shrinking''. Many open-source initiatives such as podman are gaining popularity.
...
That said, key techniques that Docker the company introduced are still widely used in all different types and brands of container runtimes and engines. It is just that people do not like some of the features of docker engine, and alternative tools are being developed to fix these problems, the latter of which starting drawing more and more attention. Docker still enjoys widespread usage and support due to its massive community, wealth of online resources, and extensive compatibility with numerous tools and platforms. 

\vspace{0.1in}
\noindent \textbf{Install Podman on RHEL}
\vspace{0.1in}

As explained earlier, RHEL uses podman as its recommended container engine. This notebook focuses on RHEL, and hence podman will be used in the remaining of the chapter. Notice that docker and podman have similar interfaces. Everything introduced in this chapter should apply to both docker and podman unless otherwise mentioned, and the command \verb|docker| and \verb|podman| can be used interchangeably. Given the popularity of docker, in this chapter \verb|docker| is used as an alias of \verb|podman|, which is a commonly adopted configuration on many servers with podman installation.
...
\vspace{0.1in}
\noindent \textbf{Install Docker on Ubuntu}
\vspace{0.1in}
...
\subsection{Manipulation} \label{sec:docker-manipulation}

This section discusses basic container manipulations such as launching and stopping a container. As mentioned earlier, command \verb|docker| is used as an alias to \verb|podman|.
...
For example, consider running a container with alpine using the command below. A screen shot is given in Fig. \ref{ch:vac:fig:dockerrunexp}.
\begin{lstlisting}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=\textwidth]{chapters/part-3/figures/dockerrunexp.png}
...
While keeping the container running, open another terminal and use \verb|docker container ls| to check the status of the container. More about container commands are collectively introduced later. The container \verb|test-alpine| shall appear in the list, as shown in Fig. \ref{ch:vac:fig:dockerrunexppart2}.
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.8\textwidth]{chapters/part-3/figures/dockerrunexppart2.png}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.8\textwidth]{chapters/part-3/figures/dockerrunexppart3.png}
...
\begin{mdframed}
\noindent \textbf{Differences between \texttt{docker run} and \texttt{docker start}}

Do note the fundamental differences between \verb|docker run| and \verb|docker start|. 
...
\end{lstlisting}
If \verb|<command>| is the shell name used in the container, this would open the terminal of the container. In the case of \verb|alpine| image based container, \verb|ash| is the default shell. For \verb|ubuntu| image based container, \verb|bash| is often used. To exit from the TTY shell while keep the container running in the backend, use shortcut key \verb|Ctrl+p+q|. An alternative way to interact with containers running in the backend is to use
\begin{lstlisting}
...
By default, containers do not expose any ports to the outside world. A container can be accessed only from its host machine using APIs provided by docker, such as \verb|docker cp| to transfer files and \verb|docker exec| to execute commands, but not network protocols. 

The user can allow public access from the internet by publishing the ports of the container using
\begin{lstlisting}
...
An example of setting up a web server in containers from scratch is given in this section. The \verb|nginx| web server is used. For simplicity, everything happens on a single physical server. Only one container is used and the load balancer and the shared services are not included in the example.

As a first step, create a container from the official \verb|nginx| image as follows. Notice that it is also possible to create a container using \verb|alpine| image then install \verb|nginx| on the container.
\begin{lstlisting}
$ docker run -dt --name simple-web nginx
\end{lstlisting}

Next, create the configuration file for \verb|nginx|, and also the HTML files to be used as the static web page. For convenience, the files are created and edited in the host machine, then copied to the container. The following \verb|default.conf| and \verb|index.html| have been created, respectively. The configuration file \verb|default.conf| is given below.
\begin{lstlisting}
server {
...
\end{lstlisting}
The HTML file \verb|index.html| is given below.
\begin{lstlisting}
<html>
	<body>
		<h1>Hello World!</h1>
...
\end{lstlisting}
Use \verb|docker copy| to copy the two files to the designed locations in the container as follows. Notice that for read-only configuration files, unlike what we are doing now, a good practice is to build them into the read-only image layers. More about docker images will be introduced in Section \ref{ch:vac:sec:di}. Keep in mind that this serves only as an demonstrative example.
\begin{lstlisting}
...
In the container, the default login user name is \verb|nginx|. Change the ownership of the HTML file as follows, so that the current user \verb|nginx| is able to access that file.
\begin{lstlisting}
...
For easy sharing and populating of the container, commit the container into a new image using \verb|docker commit|. The new image can be used to populate the web server as follows.
\begin{lstlisting}
$ docker commit simple-web simple-web-image
$ docker run -dt --name web01 -p 80:80 simple-web-image
\end{lstlisting}
where \verb|-p <host machine port>:<container port>| is used to map ports. Notice that different from the previous container, the new container \verb|web01| IP address port 80 is mapped with the port 80 of the host machine. Therefore, the web page hosted in \verb|web01| can be accessed not only by the host machine, but also by other machines in the same network with the host machine.

\section{Volume and Bind Mount} \label{ch:vac:subsec:dockervolume}

All data created or modified during the container's life cycle resides in the container's writable layer which will be introduced in detail in Section \ref{ch:vac:sec:di}. For now, just mind that if the container is removed, all data stored in its writable layer is also lost.

As introduced earlier Section \ref{sec:docker-manipulation}, when a container is running, files inside it can be accessed or copied bidirectionally using the \verb|docker cp| command manually. Therefore, it is possible that before shutting down a container, the user manually exports the data from the container into the persistent storage in the host machine. However, this is not the good practice, as it is asynchronous, redundant and involves human intervention.
...
\begin{itemize}
  \item \mync{Docker volume} is docker-managed storage. The user does not directly interact with the storage on the host machine. There are two types of docker volumes, the anonymous volume and the named volume.
  \item \mync{Bind mount} is user-managed storage. It allows the user to specify and manage the storage location on the host machine, providing direct control over the data.
\end{itemize}
...
\end{lstlisting}
Notice that docker volume is fully managed by docker. The user cannot, and does not need to specify where the data should be stored in the host machine. As far as the user concerns, it is somewhere on the host machine, and the data resides safely and does not tie to the life cycle of any particular container instance.
...
\end{lstlisting}
respectively. Notice that when using \verb|docker volume inspect|, docker gives the mount point of the volume. However, the mount point is often a place in a virtual machine that docker creates, hence not to be used to trace to the location on the host machine. Like said earlier, the user should not access the data of a volume from the host machine directly.

To remove specific an unused volume(s) or all unused volumes, use
\begin{lstlisting}
...
\end{lstlisting}
an anonymous volume will be created and used. Further more, if \verb|--rm| is used, when the container is stopped, the container together with the anonymous volume will be removed. A use case of anonymous volume is given in the later section. Anonymous volumes can also be set up in the Dockerfile.
...
Following that spirit, a developer can create a ``utility container''. They can start a container in \verb|-it| mode, install everything he needs to develop an application in that container, and develop the application there. Using bind mount, all the code they write will be mounted to his host machine bidirectionally. With this approach, he does not need to install all the dependencies of the application in his host machine.
...
\end{lstlisting} 
which mount an anonymous volume to \verb|/app/data| to protect it from being accessed or overwritten by other entities that share \verb|<volume or path 1>| with the container. This is a good use case of anonymous volume.

\section{Image} \label{ch:vac:sec:di}

\mync{Container image} performs like a blueprint that encapsulates all the necessary information needed to spawn a container. It includes initial configurations, requisite libraries, and other pertinent metadata. Docker images are highly portable and can be shared across various machines and platforms. Container instances are most often created from container images.

This section delves deeper into the construction and functionality of container images. Notice that docker image is used interchangeably with container image in the remainder of this section. 
...
\begin{itemize}
	...
	\item Files, data, source codes, configurations, etc., for the application
  	\item Steps to integrate and deploy the libraries, tools, dependencies, files, data, source code, configurations, etc.
  	\item Startup commands
\end{itemize}
In addition, an image shall be designed and organized in such a way that it is portable, reusable and light, and can be used to easily populate large number of containers. For better inheritability, an image might be based on another existing image, in which case the image it bases on is known as the \mync{parent image}. An image with no parent is called a \mync{base image}.
...
\begin{enumerate}[(1)]
	...
	\item Add files, data, source codes, configurations, etc.
	\begin{itemize}
		\item Setup file system
		\item Install dependencies and other programs
		\item Include files, data, source code and configurations
	\end{itemize}
	\item Specify startup command
\end{enumerate}
A more detailed step-by-step guidance example is given later. The relationship between Dockerfile, image and container is illustrated in Fig. \ref{ch:vac:fig:dockerfiletoimage}. Notably, the Dockerfile itself is not included within the resulting image.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.8\textwidth]{chapters/part-3/figures/dockerfiletoimage.png}
...
Docker containers employ a special file system known as the Union File System (UFS), which is well suited to the ``layer'' concept. UFS facilitates file sharing between the container and the host machine, along with combining read-only upper layers and writable lower layers, among other functions. A demonstration to illustrate this concept is given in Fig. \ref{ch:vac:fig:dockerlayerdemo}.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=\textwidth]{chapters/part-3/figures/dockerlayerdemo.png}
	\caption{A demonstration of docker image layer structure.} \label{ch:vac:fig:dockerlayerdemo}
\end{figure}

Just as a quick example, the Dockerfile to build the official \verb|hello-world| image from docker hub looks like the following.
\begin{lstlisting}
FROM scratch
COPY hello /
CMD ["/hello"]
\end{lstlisting}
In the example above, \verb|FROM scratch| signifies that this image is a base image without a parent. The \verb|COPY hello /| instruction copies the \verb|hello| binary script from the image to the root directory of the container. Lastly, \verb|CMD ["/hello"]| runs the \verb|hello| binary script.
...
\begin{enumerate}[(1)]
  \item Define base image.
  \item Create filesystem directory.
  \item Set working directory.
  \item Install libraries, dependencies, tools and applications.
  \begin{enumerate}[(a)]
  	\item Add / Copy files.
  	\item Add registry.
  	\item Install libraries, dependencies, tools and applications.
  ...
  \item Expose port.
  \item Start the application using user script.
\end{enumerate}
...
It is worth highlighting the difference between \verb|RUN| and \verb|CMD|. The \verb|RUN| command is executed during the building of the image, which creates a virtual representation of the environment to run the service. Therefore, \verb|RUN| is used to prepare the environment, for example, to install necessary libraries. On the other hand, command \verb|CMD| simply specifies the default command to run when the container starts. It is built together with the image, but can be overwritten if the user wishes to do so when running \verb|docker run|. In this sense, though \verb|CMD| instruction is included in the image, it is not quite read-only.
...
\subsection{Image Building} \label{sec:docker-image-building}

With the Dockerfile ready, use \verb|docker build| to build an image. An example is given as follows.
\begin{lstlisting}
$ docker build <path/url> -t [<username>/]<image name>[:<tag>]
\end{lstlisting}
where \verb|<path/url>| is the path or URL to the directory where the Dockerfile is located at. Notice that it is assumed that the Dockerfile is named \verb|Dockerfile| (with no suffix), and ``\verb|/Dockerfile|'' does not need to be included in \verb|<path/url>|. Terms \verb|<username>/| and \verb|:<tag>| are optional as part of the image name. For the convenience of sharing, it is recommended that all images to be pushed to a public registry (such as Docker Hub) shall be tagged with username and version. Notice that the tag can be changed later after the image is built.

It is worth mentioning that \verb|docker build| builds an image by layers in the Dockerfile, and it will not re-build the same layer to which point no changes are made. This feature suggests that we should put the common and static layers in the early part of Dockerfile, and customized and user-defined layers in the late part, thus making the building (and also sharing, as will be introduced later) of the images more efficient.
...
\end{lstlisting}
The idea behind multi-stage image built is as follows.
\begin{itemize}
...
The first two operations have been introduced in earlier Sections \ref{sec:docker-image-building} and \ref{sec:docker-manipulation}. The third and last ones are introduced below. Use the following command to search for an image on the default remote repository server, which is often Docker Hub.
\begin{lstlisting}
$ docker search <image name>
\end{lstlisting}
Use the following command to download or update an image from the default remote repository server. Notice that different from \verb|docker run|, this command will not start a container from the image after the download.
\begin{lstlisting}
...
Docker Hub is a commonly used server for storing and sharing docker images. It is also the default remote repository server of docker engine. However, do notice that Docker Hub is not the only remote docker image server. Some alternatives are Amazon Elastic Container Registry, Red hat Quay, Azure Container Registry, Google Container Registry, etc. In the remainder of this section, only Docker Hub is concerned.

After registering an account on Docker Hub, use the following command to login to the Docker Hub from a local machine.
\begin{lstlisting}
$ docker login --username=<user name>
Passowrd:
\end{lstlisting}

Assume that there is an image in the local machine, and an empty repository on Docker Hub. In order to push the local image to the Docker Hub, the first step is to add the remote repository and the the repository tag in the local image as follows.
\begin{lstlisting}
...
As introduced earlier, docker engine can be used to build and share images as well as start, monitor, and stop containers. It can be difficult for a user to manage containers manually when a lot of them are deployed. Container orchestrator such as Portainer and Kubernetes are helpful with managing containers. Many of these tools are able to automatically adjust the number of containers and balance their loads.
