...
The data is stored in the format of \mync{binary JSON}[BSON]. BSON is an extension of JSON that supports more data types. MongoDB uses BSON to store and process data due to its enhanced capability, and uses JSON to display the data.
...
\begin{mdframed}
\noindent \textbf{Should I use a semicolon in the end of each MongoDB command?}
...
\end{lstlisting}
the semicolon is highly recommended by JavaScript. Although in JavaScript semicolon has become optional due to \myabb{Automatic Semicolon Insertion}{ASI}, it is still widely recommended to use semicolon anyway.
