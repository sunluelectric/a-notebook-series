...
\mync{MongoDB} is a source-available, cross-platform, general-purpose, document-oriented NoSQL database program. It stores data in documents with flexible schema.
...
MongoDB is a document-oriented database program and it stores data in documents and collections instead of rows and tables. A \mync{MongoDB document} is the basic unit of the storage. A \mync{MongoDB collection} is a group of MongoDB documents on the same/similar topic. Each document in the same collection can adopt its own schema, and it should be self-contained so that when querying the data the user does not need to join collections and map documents. 

Figure \ref{ch:database:mongotree} gives a demonstrative example of how MongoDB stores and organizes data. In this example, a collection ``USER'' is defined, and it contains two documents. Each document has several properties. Different document may share common properties such as ``name'' in this example. Each document can also have unique properties of its own such as ``age'', ``address'' and ``language''. Different properties may have different data types. For example, a property can be string, numeric, or a nested object, or an array of the above. When querying MongoDB, the DBMS is able to return selected properties of documents that meet specific criteria, and sort them in the required order.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{chapters/part-3/figures/mongodb_tree.png}
	\caption{A demonstrative example of how MongoDB stores data as (nested) object.} \label{ch:database:mongotree}
\end{figure}

The data is stored in the format of \mync{binary JSON}[BSON]. BSON is an extension of JSON that supports more data types. MongoDB uses BSON to store and process data due to its enhanced capability, and supports both BSON and JSON when comes to data importing and exporting.

\begin{mdframed}
\noindent \textbf{JSON VS BSON}
...
BSON, on the other hand, is binary-based and hence contains more information. In BSON, more data types are supported, such as different floats, date, timestamp, binary string, regular expression and code block. This makes BSON more powerful and efficient than JSON. As a compromise, BSON is less intuitive and human-readable than JSON.
	
\end{mdframed}

It is strongly recommended to make the data (document and collection) self-contained. The MongoDB architecture design should base on how the data is to be queried. Indeed, relevant information is often queried together and hence should reside together in the storage. This concept is known as \mync{embedding}. Despite making each collection and document slightly bigger, embedding makes the query simpler. Do note that when performing embedding, make sure that the size of the document is bounded. MongoDB has a maximum $16$MB size limitation for each document.
...
MongoDB has both community and enterprise versions. Under the scope of this notebook, the community version is concerned. MongoDB community server can be installed following the instructions on the official website at \cite{mongodb_community_server}. Different versions of MongoDB are available. Choose the correct MongoDB version depending on the CPU and the OS of the machine. MongoDB community server installation size is about $500$MB. A MongoDB server can host a MongoDB cluster, inside which are many MongoDB databases. Each database can have its own configurations and policies.

To access MongoDB, it is recommended that MongoDB Shell, the \myabb{command line interface}{CLI} of MongoDB, be installed on the user machine. More about MongoDB Shell will be introduced in Section \ref{sec:mongosh}. It is also possible to use MongoDB Compass, the recommended \myabb{graphical user interface}{GUI} for MongoDB, to interact with MongoDB. MongoDB Compass can be used to connect to a MongoDB server and view and manipulate the databases on the server. It also comes with analytical tools and data visualization tools that help the user understand the insight of the data.
...
\section{MongoDB Shell} \label{sec:mongosh}

To interact with MongoDB, the quickest way is to use MongoDB shell, which is introduced in this section. MongoDB shell, if not installed together with the MongoDB server, can be found at \cite{mongodb_shell_download}. 

After installing both MongoDB server and MongoDB shell, use \verb|mongosh| to enter the MongoDB shell console. JavaScript-like commands are used to manipulate the database, such as creating databases and inserting data. The object \verb|db| contains many methods using which the user can access and modify the basic configurations of the database. For example,
\begin{lstlisting}
> db.version()
\end{lstlisting}
gives the version of the database server. Some commonly used commands are listed in Table \ref{ch:db:tab:mongodbbasics}.
\begin{table}[!htb]
...
Commonly used database manipulation commands are introduced in the remainder of the section.

\subsection{Database Selection}

To display existing databases, use
\begin{lstlisting}
> show dbs
\end{lstlisting}
...
To drop a database, switch to the database and use 
\begin{lstlisting}
> db.dropDatabase()
\end{lstlisting}

\subsection{Collection and Document Creation}

A MongoDB database contains one or more collections. A collection is similar to a table in an RDB in the sense that it is the collection of data on the same / similar topic. However, unlike tables where schematics such as column names and data types are enforced upon creation of the table, a collection does not enforce fields and their data types. 

To create a document, switch to the target database then use
\begin{lstlisting}
db.createCollection(<name>[,<options>])
\end{lstlisting}
where \verb|<options>| is an optional JSON-like string that allows creating different types of collections, such as time-series collection which is optimized to store time series sensor data. In the demonstrative example below, commonly seen fields in \verb|<options>| are shown.
\begin{lstlisting}
...
After creating a collection, use
\begin{lstlisting}
> show collections
\end{lstlisting}
to show the collections in the current database.

\begin{mdframed}
\noindent \textbf{Should semicolon be used in the end of each MongoDB command?}

No. It works both ways. For example, to select a database, in MongoDB shell both
\begin{lstlisting}
> use myNewDatabase
\end{lstlisting}
and
...
However, if JavaScript environment such as \textit{Node.js} is used and MongoDB commands are integrated as part of the program, it is recommended to use semicolon. For example, in \textit{Node.js} selecting a database would look like the following
\begin{lstlisting}
const newDb = client.db("myNewDatabase");
\end{lstlisting}
in which case the semicolon is highly recommended. Although in JavaScript semicolon has become optional due to \myabb{Automatic Semicolon Insertion}{ASI}, it is still widely recommended to use semicolon anyway.

To conclude, semicolon is recommended just to follow the general JavaScript good practice. 
\end{mdframed}
...
Notice that user ``Alice'' and ``Bob'' in the above example are represented by JSON and BSON respectively. MongoDB uses BSON internally, but it can also take JSON as input, in which case MongoDB converts JSON to BSON internally.
...
To retrieve all the documents of a collection, use
\begin{lstlisting}
...
\begin{mdframed}
\noindent \textbf{Dynamic Naming of a Collection}
...
Query can be applied to elements in an array. A commonly used operator is \verb|$elemMatch|.
\begin{lstlisting}
...
\subsection{Document Removal}

Use either \verb|deleteOne(<filter>)| or \verb|deleteMany(<filter>)| to remove documents, with the input the filter document.
...
\begin{mdframed}
\noindent \textbf{MongoDB's B-Tree}
...
Here is a quick example to connect to MongoDB using Python library \verb|PyMongo|.
\begin{lstlisting}
import pymongo
from pymongo import MongoClient
...
\section{MongoDB Atlas} \label{sec:mongodb-atlas}
