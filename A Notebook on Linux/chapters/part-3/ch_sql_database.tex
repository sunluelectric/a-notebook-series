...
Different databases and database manage systems are introduced in this and the next few chapters. Both relational and non-relational databases are covered.
...
\mync{Relational database}[RDB] was proposed in 1970s by IBM. Some important features of a relational database include the following.
\begin{itemize}
...
\mync{Structured Query Language}[SQL], a domain-specific language, can be used in managing RDB.  Most commercialized \myabb{RDB management systems}{RDBMS} adopt SQL as the database query and manipulation language. There are alternatives, but they are rarely used compared to SQL. There is an evolving standard on what operations should an RDBMS support using SQL. The latest version as of this writing is \verb|SQL:2023|.
...
\section{Structured Query Language} \label{sec:sql}
...
\item Table:
\begin{itemize}
	\item Avoid using the same name for a table and one of its columns.
	\item Avoid concatenating two table names as a third table name.
\end{itemize}
\item Column:
\begin{itemize}
	\item Use singular name for columns.
	\item Avoid using over simplified and not self-explanatory terms.
...
\item suffixes: the following suffix are often used with conventional meanings as follows.
\begin{itemize}
...
\begin{table}[!htb]
	...
	\begin{tabularx}{\textwidth}{lX}
		...
		VIEW & A view, which is a virtual table that does not store data, but reflects the result of a query in the table format. \\ 
