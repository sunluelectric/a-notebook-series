\chapter{Optimal Control System} \label{ch:optimalrobustcs}

In a typical optimal control problem formulation, a cost function is defined. The cost function is often made positively correlated with the regulation error and the control signal energy. Such control that minimizes the cost function is calculated. The key to the optimal control is to formulate the problem appropriately and to find the correct mathematical tools to solve the problem. 

Optimization problem formulation and optimal control are introduced in this chapter.

\section{Static Optimization} \label{sec:optimalrobustcs:static}

Assume static model where the cost function $L(u)$ (or $L$ for simplicity) is a function of $u$ and does not change over time. There may or may not be additional constraints on $u$. If there are constraints, the constraints do not change over time either. The objective is to find the signal $u$ that minimizes $L$. 

Notice that $u\in\mathbb{R}^m$ is a vector containing $m$ independent variables, and $L\in\mathbb{R}$ is a scalar. It is assumed that $L$ is smooth, and it has derivatives of all orders on $u$.

\subsection{Optimization without Constraint}

Consider minimizing $L$ without additional constraints on $u$. Using Taylor series expansion, the increment of $L$ can be formulated as follows.
\begin{eqnarray}
	\Delta L &=& L_u^T\Delta u + \dfrac{1}{2}\Delta u^TL_{uu}\Delta u + \ldots \nonumber
\end{eqnarray}
where
\begin{eqnarray}
	L_u &=& \dfrac{d L}{d u} \in \mathbb{R}^m \label{eq:lu} \\
	L_{uu} &=& \dfrac{d^2 L}{d u^2} \in \mathbb{R}^{m\times m} \nonumber
\end{eqnarray}
about a particular choice of $u$. A local minimum of $L$ is achieved under the following necessary condition
\begin{eqnarray}
	L_u &=& 0 \nonumber
\end{eqnarray}
in which case $u$ is called a \mync{critical point}, also known as \mync{stationary point}. Furthermore, for $u$ to be a minimum, $\Delta L$ should stay positive in the neighbor of $u$. A necessary condition for that could be that $L_{uu}$ is positive definite, i.e.,
\begin{eqnarray}
	L_{uu} &>& 0 \nonumber
\end{eqnarray}
...
\begin{subequations}\label{eq:optimization-equality-constraint}
\begin{eqnarray}
	\textup{minimize} && L(u) \label{eq:optimalrobustcs:equalityconsfunc} \\
	\textup{s.t.} && f(u) = 0 \label{eq:optimalrobustcs:equalityconstraint}
\end{eqnarray}
\end{subequations}
where $u\in\mathbb{R}^m$ and $f(u)\in\mathbb{R}^n$ (or $f$ for simplicity), and without losing generality we assume that $n<m$ (otherwise $u$ might be solvable from $f=0$).

It is obvious that the solution to \eqref{eq:optimalrobustcs:equalityconsfunc} would not be necessarily at $L_u=0$. As a matter of fact, at any point of $u$ satisfying $f=0$, it is not necessary that $L_u=0$. This is demonstrated by Fig. \ref{fig:optimalrobustcs:opt_equalityconstraint_contour},
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.6\textwidth]{chapters/part-3/figures/opt_equalityconstraint_contour.eps}
	\caption{A demonstration of static optimization with equality constraint problem formulation.} \label{fig:optimalrobustcs:opt_equalityconstraint_contour}
\end{figure}
where $L$ is given by the contour (assuming $u\in\mathbb{R}^2$ in this demonstration plot), and $f=0$ the red solid line.

A necessary condition of the critical point of \eqref{eq:optimalrobustcs:equalityconsfunc} subject to \eqref{eq:optimalrobustcs:equalityconstraint} can be interpreted as follows. 

Let $du$ be a differential about $u$ where $f = 0$. Let $L_u$ be given by \eqref{eq:lu} and $f_u = \frac{df}{du}$ the Jacobin matrix. While $dL = L_u^Tdu \neq 0$ in general, when $du$ moves along $f=0$, i.e., when $df = f_ud_u = 0$, $dL = L_u^Tdu = 0$ should hold true. This is expressed by
...
A necessary condition for \eqref{eq:equalitycritical} is given as follows. There must be such $\lambda \in \mathbb{R}^n$ with $n$ degrees of freedom that
\begin{eqnarray}
...
In \eqref{eq:optimalrobustcs:equalitycritical4}, $\lambda \in \mathbb{R}^n$ is known as the \mync{Lagrange multipliers}. Equation \eqref{eq:optimalrobustcs:equalitycritical4} contains $m$ equations, and it is a portion of \myabb{Karush–Kuhn–Tucker}{KKT} conditions which will be introduced formally in Section \ref{sec:optimization-lagraigian-eq-ineq}.

Equation \eqref{eq:optimalrobustcs:equalitycritical4} can also be interpreted graphically. At the critical point denoted by the red dot, moving along $f=0$ should not increase or decrease $L$. This suggests that $f=0$ shall tangent the contour at the critical point as shown in Fig. \ref{fig:optimalrobustcs:opt_equalityconstraint_graphical}.
\begin{figure}[!htb]
	\centering
	\includegraphics[width=300pt]{chapters/part-3/figures/opt_equalityconstraint_graphical.png}
...
\end{figure}
Think of $f_u=\nabla f$ and $L_u=\nabla L$ as the gradient of $f$ and $L$ respectively. Since the plots are tangent, the gradient must be parallel, which would suggest something like \eqref{eq:optimalrobustcs:equalitycritical4}.

The necessary condition \eqref{eq:optimalrobustcs:equalityconstraint} and \eqref{eq:optimalrobustcs:equalitycritical4} can also be formulated using \mync{Lagrangian function} which is given below. Notice that Lagrangian function in some context may also be called \mync{Hamiltonian function}. They are very similar mathematically, and mostly differ in the context of usage.

The Lagrangian function corresponding with problem \eqref{eq:optimization-equality-constraint} is given by
\begin{eqnarray}
...
Consider the following example.
\begin{shortbox}
\Boxhead{A Quadratic Surface with Linear Equality Constraint Example}
...
Consider the optimization problem with both equality and inequality constraints as follows.
\begin{eqnarray}
	\textup{minimize} && L(u) \nonumber \\
	\textup{s.t.} && f(u) = 0 \nonumber \\
...
\begin{itemize}
  ...
  \item Equation \eqref{eq:optimalrobustcs:kkt3} implies that one of $\beta_i$ and $g_i(u)$ must be zero. Equations \eqref{eq:optimalrobustcs:kkt3}, \eqref{eq:optimalrobustcs:kkt4} and \eqref{eq:optimalrobustcs:kkt5} together interpret the inequality constraint as follows.
  \begin{itemize}
...
\section{Discrete Optimal Control Basics} \label{sec:optimalrobustcs:discrete}

Optimal control is strongly connected with the static optimization problem formulation introduced in Section \ref{sec:optimalrobustcs:static}. In a broad sense, optimal control can be taken as a use case of the static problem formulation, with the system model treated as a constraint.

This section introduces the basics of optimal control on a discrete time system. The next Section \ref{sec:continuous-optimal-control-basics} introduces that of a continuous time system.
...
\subsection{Discrete Linear Quadratic Regulator}

\mync{Linear quadratic regulator}[LQR] is a special case of the general problem formulation \eqref{eq:optimalrobustcs:generaldynamic} and \eqref{eq:optimalrobustcs:generaldiscrete}. It is one of the most commonly seen optimal control problems.
...
\subsection{Suboptimal Discrete Linear Quadratic Regulator} \label{subsec:suboptimaldiscretelqr}
...
\section{Continuous Optimal Control Basics} \label{sec:continuous-optimal-control-basics}
...
\subsection{Continuous Linear Quadratic Regulator}
...
\subsection{Suboptimal Continuous Linear Quadratic Regulator}
...
\section{Linear Quadratic Regulator Extensions}
