...
In a typical optimal control problem formulation, a cost function positively correlated with regulation error, control signal energy, etc., is defined. Such control that minimizes the cost function is calculated. The key to the optimal control is to formulate the problem appropriately and to find the correct mathematical tools to solve the problem. 

More details are introduced in this chapter.
...
\begin{eqnarray}
	L_u &=& 0 \nonumber
\end{eqnarray}
in which case $u$ is called a \mync{critical point}, also known as \mync{stationary point}. Furthermore, for $u$ to be a minimum, $dL$ should stay positive in the neighbor of $u$. A necessary condition for that could be that $L_{uu}$ is positive definite, i.e.,
\begin{eqnarray}
	L_{uu} &>& 0 \nonumber
\end{eqnarray}
...
Let $du$ be a differential about $u$ where $f = 0$. Let $L_u$ be given by \eqref{eq:lu} and $f_u = \frac{df}{du}$ the Jacobian matrix. While $dL = L_u^Tdu \neq 0$ in general, when $du$ moves along $f=0$, i.e., when $df = f_ud_u = 0$, $dL = L_u^Tdu = 0$ should hold true. This is expressed by
...
\section{Continuous Optimal Control Basics}
