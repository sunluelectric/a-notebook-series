...
Model predictive control system belongs to the broader family of optimal control system. It uses a model to forecast the system behavior, and computes such control sequence that optimizes the forecast while fulfilling all constraints. The optimization problem is solved recurrently at each timestamp, with only the first control from the optimal sequence actually applied.

Due to its popularity, model predictive control system is introduced in this dedicated chapter but not in Chapter \ref{ch:optimalrobustcs}.

\section{Introduction}

A \mync{model predictive control}[MPC] system uses a dynamic model to predict the behavior of the plant, and then find the control sequence that minimizes the cost function formulated from the forecast to produce the best decision, meanwhile fulfilling all the constraints applied to the state and control.

MPC is often compared with \myabb{linear quadratic regulator}{LQR} which has been introduced in Chapter \ref{ch:optimalrobustcs}. Indeed, they are similar to some extent. For instance, they are both parametric model based optimization control. MPC is often regarded as a more robust and flexible solution than LQR, but with a higher computation burden. Their differences will be discussed in detail in the remainder of the chapter. 
...
\section{State-Space Model Predictive Control}

TBA

\section{Input-Output Model Predictive Control}
