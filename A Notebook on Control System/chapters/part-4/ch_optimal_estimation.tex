\chapter{Optimal State Estimation} \label{ch:orse}
...
UKF was proposed to address item 1 in the above list. Notice that UKF still assumes Gaussian distributed process and measurement noise, and it is not robust to outliers.
...
\begin{eqnarray}
	...
	P_{yy}(k) &=& \sum_{i=0}^{2L-1} w_i^c \left[g_k\left(f_{k-1}\left(\mathcal{X}_i^x, \mathcal{X}_i^w\right), \mathcal{X}_i^v\right) - \hat{y}(k)\right] \times \ldots \nonumber \\
	&& \left[g_k\left(f_{k-1}\left(\mathcal{X}_i^x, \mathcal{X}_i^w\right), \mathcal{X}_i^v\right) - \hat{y}(k)\right]^T \label{eq:ukfpyy} \\
	P_{xy}(k) &=& \sum_{i=0}^{2L-1} w_i^c \left[\mathcal{X}_i^x -\hat{x}^-(k)\right] \times \ldots \nonumber \\
	&& \left[g_k\left(f_{k-1}\left(\mathcal{X}_i^x, \mathcal{X}_i^w\right), \mathcal{X}_i^v\right) - \hat{y}(k)\right]^T \label{eq:ukfpxy}
\end{eqnarray}
...
\begin{tikzpicture}[node distance=0.6in]
	...
	\node (pro4) [process, below of=pro3] {Calculate $\hat{x}^-(k)$ and $P^-(k)$ using \eqref{eq:ukftimex} and \eqref{eq:ukftimep}};
	\node (in1) [io, below of=pro4] {Receive $y(k)$};
	\node (pro5) [process, below of=in1] {Calculate Kalman gain using \eqref{eq:ukfyhat}, \eqref{eq:ukfpyy}, \eqref{eq:ukfpxy} and \eqref{eq:ukfkg}};
	\node (pro6) [process, below of=pro5] {Calculate $\hat{x}(k)$ and $P(k)$ using \eqref{eq:ukfmeasurementx} and \eqref{eq:ukfmeasurementp}};
	\node (out1) [io, below of=pro6] {Output $\hat{x}(k)$};
	\node (dec1) [decision, below of=out1, yshift=0in] {End?};
	\node (pro2b) [process, right of=dec1, xshift=2.5in] {$k \leftarrow k + 1$};
